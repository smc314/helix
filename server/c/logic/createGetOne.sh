...
CLASSNAME=GetOne${OBJNAME}
HEADERFILE=GetOne${OBJNAME}.h
BODYFILE=GetOne${OBJNAME}.cpp
...
void ${CLASSNAME}::ExecuteRequest(IOConn& ioc)
{
	...
	${OBJNAME}_svect vect = ${OBJNAME}::selectById( db, local.id );
	if(vect->size() == 0){
		throw AnException(0, FL, "Unknown ${OBJNAME} object: %d", local.id);
	}
