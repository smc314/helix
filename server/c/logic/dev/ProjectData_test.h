...
#ifndef PROJECTDATA_TEST_H
#define PROJECTDATA_TEST_H
...
/** This is a generated data object test class that is used for testing the data
  * object called ProjectData.
  */
class ProjectDataTest : public DataObjectTestClass
{
	private:
		/// Copy constructor is private to prevent use
		ProjectDataTest(const ProjectDataTest& c) {}

		/// Assignment operator is private to prevent use
		ProjectDataTest& operator=(const ProjectDataTest& c) { return *this; }

		/// Registers us with the global factory map
		static DataObjectTestClassRegister< ProjectDataTest > reg;
	...
	public:
		...
		static bool compareObjects( ProjectData* first, ProjectData* second);

		/// Compare two lists of objects to see if they are the same
		static bool compareLists( vector< ProjectData* >* first, vector< ProjectData* >* second);
		...
		/** This method will use our input XML document to prepare for and test
		  * the ProjectData.insert method.  We will use our list of output
		...
		/** This method will use our input XML document to prepare for and test
		  * the ProjectData.selectAllForProj method.  We will use our list of output
		  * xml documents to verify the output of this method.  If the list of ourput
		  * documents is empty, we will simply ensure that no exceptions are thrown using
		  * the given XML input document.
		  */
		void selectAllForProj(IOConn& ioc, xmlNodePtr node);

		/** This method will use our input XML document to prepare for and test
		  * the ProjectData.selectByID method.  We will use our list of output
		  * xml documents to verify the output of this method.  If the list of ourput
		  * documents is empty, we will simply ensure that no exceptions are thrown using
		  * the given XML input document.
		  */
...
#endif // PROJECTDATA_TEST_H defined
