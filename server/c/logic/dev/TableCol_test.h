...
#ifndef TABLECOL_TEST_H
#define TABLECOL_TEST_H
...
/** This is a generated data object test class that is used for testing the data
  * object called TableCol.
  */
class TableColTest : public DataObjectTestClass
{
	private:
		/// Copy constructor is private to prevent use
		TableColTest(const TableColTest& c) {}

		/// Assignment operator is private to prevent use
		TableColTest& operator=(const TableColTest& c) { return *this; }

		/// Registers us with the global factory map
		static DataObjectTestClassRegister< TableColTest > reg;
	...
	public:
		...
		static bool compareObjects( TableCol* first, TableCol* second);

		/// Compare two lists of objects to see if they are the same
		static bool compareLists( vector< TableCol* >* first, vector< TableCol* >* second);
		...
		/** This method will use our input XML document to prepare for and test
		  * the TableCol.insert method.  We will use our list of output
		...
		/** This method will use our input XML document to prepare for and test
		  ...
		  * the given XML input document.
		  */
		void deleteByTable(IOConn& ioc, xmlNodePtr node);

		/** This method will use our input XML document to prepare for and test
		  * the TableCol.selectAllForTable method.  We will use our list of output
		  * xml documents to verify the output of this method.  If the list of ourput
		  * documents is empty, we will simply ensure that no exceptions are thrown using
		  * the given XML input document.
		  */
		void selectAllForTable(IOConn& ioc, xmlNodePtr node);
