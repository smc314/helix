...
CLASSNAME=GetAll${OBJNAME}
HEADERFILE=GetAll${OBJNAME}.h
BODYFILE=GetAll${OBJNAME}.cpp
...
void ${CLASSNAME}::ExecuteRequest(IOConn& ioc)
{
	...
	${OBJNAME}_svect vect = ${OBJNAME}::selectAll( sqldb );

	// Add the selected data to the response document
	${OBJNAME}::createXmlChildren( ioc.getResponseRoot(), vect );

	// Example of a doing this through an odbc connection
	// OdbcObj& odbc = *ioc.getDBConnection();
	// 
	// ${OBJNAME}_svect vect = ${OBJNAME}::selectAll( odbc );
