...
void ApiTest_Authenticate_NoAuthorization_ShouldFail();
void ApiTest_Authenticate_CallWithEmptyPayload();
void ApiTest_Authenticate_CallWithValidPayload();
void ApiTest_Authenticate_CallWithInvalidPayload();

// Define more of your own tests and methods for testing /Authenticate here

/** This is the main method that executes the tests for the /Authenticate api.  All tests
  * that should be executed against this api should be invoked directly here.
  */
void ApiTest_Authenticate()
{
	// Ensure that we should be testing this API
	if(runTestsForApi("/Authenticate") == false){
		return; // Tests for this api are not included in this run.
	}

	// Test the Api without authorization:
	ApiTest_Authenticate_NoAuthorization_ShouldFail();

	// Test the Api with an empty payload
	ApiTest_Authenticate_CallWithEmptyPayload();

	// Test the Api with a valid payload
	ApiTest_Authenticate_CallWithValidPayload();

	// Test the Api with an invalid payload
	ApiTest_Authenticate_CallWithInvalidPayload();

	// Call any other tests that you have defined here:
}

/** This will call the /Authenticate without any authorization information.  This should
  * be rejected by the server, and we should receive the error message.
  */
void ApiTest_Authenticate_NoAuthorization_ShouldFail()
{
	BEGIN_TEST_METHOD( "ApiTest_Authenticate_NoAuthorization_ShouldFail" )

	// Remove this out when you've updated these tests to be real
	ASSERT_TRUE(false, "Test not implemented yet.");

	// Api /Authenticate has no input values.
	xmlDocPtr resp;
...
/** This test will call the /Authenticate with an empty payload and check the response
  * that is received.
  */
void ApiTest_Authenticate_CallWithEmptyPayload()
{
	BEGIN_TEST_METHOD( "ApiTest_Authenticate_CallWithEmptyPayload" )
...
/** This test will call the /Authenticate with a valid payload and check the response
  * that is received.
  */
void ApiTest_Authenticate_CallWithValidPayload()
{
	BEGIN_TEST_METHOD( "ApiTest_Authenticate_CallWithValidPayload" )
...
/** This test will call the /Authenticate with an invalid payload and check the response
  * that is received.
  */
void ApiTest_Authenticate_CallWithInvalidPayload()
{
	BEGIN_TEST_METHOD( "ApiTest_Authenticate_CallWithInvalidPayload" )
