 ...
 * <tt>test</tt> folder.
 */
qx.Class.define("PACKAGE.test.HelixEditorTest",
{
	extend : PACKAGE.test.HubTest,
	type : "abstract",
	...
	members :
	{
		...
		test0005_deleteOldNode: function() {
			...
			var editorUtils = new PACKAGE.test.EditorTestUtil( this,
					                                       this._treeNode,
														   this._tabNameNew,
					                                       this._tabNameSaved );
		...
		test0010_OpenNewEditor: function() {
			PACKAGE.test.TestStatics.selectToolsMenuItem( "New", this._toolsNew );
		...
		test0080_OpenEditor: function() {
			qx.event.Timer.once( function() { // avoid's resume called before wait when the app is fast
		...
		test0100_DeleteNewObject : function() {
			...
			this.getFileTreeNode( treePath, function(treeNode) {
				...
				PACKAGE.test.TestStatics.waitForDialog( "Deleting...", function(dialog) {
					dialog.ok_btn.execute();
