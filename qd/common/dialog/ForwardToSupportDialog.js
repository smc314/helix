...
/** This class defines a dialog that presents choice for the user in how they want to forward
  ...
  */
qx.Class.define("PACKAGE.dialog.ForwardToSupportDialog", {
	extend : PACKAGE.dialog.CancelDialog,
	...
	construct : function ( forwardToSupportFileName ) {
		...
		this.icon = "PACKAGE/icon/64x64/shadow/user_headset.png";
		...
		this.downloadBtn = this.addButton( "Download Zip", "PACKAGE/icon/16x16/plain/download.png" );
		this.forwardBtn = this.addButton( "Use Direct Send", "PACKAGE/icon/16x16/plain/server_from_client.png" );
	...
	members : {
		...
		handleForward: function() {
			var fileObj = new PACKAGE.sqldo.SupportBundle;
			fileObj.setFileName( this.forwardFile );
			PACKAGE.Api.ForwardFileToSupport( fileObj, function(response) {
				PACKAGE.Statics.doAlertGreen(
