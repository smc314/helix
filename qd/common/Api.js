...
************************************************************************ */
/*
#asset(PACKAGE/ajax-loader-32.gif)
#asset(PACKAGE/ajax-loader-16.gif)
#asset(PACKAGE/icon/32x32/shadow/security_badge.png)

*/
qx.Class.define("PACKAGE.Api", {

	type : "static",

	statics : {
		/** This function will walk a PACKAGE response XML document and look for
		...
		returnHasErrors : function ( response, doAlert, callback, theThis ) {
			if(response == null){
				if(doAlert){
					PACKAGE.Statics.doAlert("xmlhttp response is null!");
				}
				return true;
			}
			...
			if(error_response !== null && error_response === "true"){
				if( doAlert ){
					...
					if(errors.length > 1){
						PACKAGE.Statics.doAlert("Errors occurred during processing:<br/><br/>" + 
							PACKAGE.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					} else {
						PACKAGE.Statics.doAlert("Error occurred during processing:<br/><br/>" + 
							PACKAGE.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					}
				}
				return true;
		...
		returnIsLong : function(response, callback, theThis, errorCallback){
			...
			var lrTask = new PACKAGE.sqldo.LRTask();
			lrTask.readFromXML( response );
			var tmpWindow = new PACKAGE.dialog.LRTaskDialog( lrTask, callback, errorCallback, theThis );
		...
		activateNetworkIcon : function ( theThis ){
			...
			if(theThis && theThis.toolbar && theThis.serverMenu ){
				theThis.serverMenu.setIcon("PACKAGE/ajax-loader-16.gif");
			}
		},
		...
		deactivateNetworkIcon : function ( theThis ){
			...
			if(theThis && theThis.toolbar && theThis.serverMenu){
				if (qx.lang.String.contains(theThis.serverMenu.getLabel(), "Not Connected")) {
					theThis.serverMenu.setIcon("PACKAGE/icon/16x16/plain/server_delete.png");
				} else {
					theThis.serverMenu.setIcon("PACKAGE/icon/16x16/plain/server_connection.png");
				}
			}
		},
		...
		SendRequest : function( requestDoc, requestName, callback, theThis, errorCallback) {
			PACKAGE.Api.activateNetworkIcon( theThis );
			...
			req.onreadystatechange = qx.lang.Function.bind(function() {
				...
				if(PACKAGE.Api.CheckNullResponse( requestName, req, theThis ) === true){
					...
					if(theThis.ApiNestLevel > 5){
						PACKAGE.Statics.doAlert("Error communicating with server.<br/><br/>Request: " +
					...
					PACKAGE.Api.SendRequest( requestDoc, requestName, callback, theThis, errorCallback);
					return;
				}
				
				var response = req.responseXML.documentElement;

				if( PACKAGE.Api.returnRequiresDB(response) ){
					// User has not yet chosen a server connection.  Prompt them to do so.
					PACKAGE.Api.ReAuthenticate(requestDoc, requestName, callback, theThis);
					return;
				}
				
				if(PACKAGE.Api.returnHasErrors( response, true, errorCallback, theThis) ){
					if(theThis){
						theThis.fireEvent( "apiCallFailed" );
					}
					return;
				...
				if(PACKAGE.Api.returnIsLong( response, callback, theThis, errorCallback ) ) {
					return;
				}
		...
		LoadXMLDoc : function( requestName, callback, theThis, errorCallback) {
			...
			req.onreadystatechange = qx.lang.Function.bind(function() {
				...
				PACKAGE.Api.deactivateNetworkIcon( theThis );
				if(req.responseXML === null && req.responseText.indexOf("PACKAGE_LOG_ON") !== -1){
					// session has expired.  Re-authenticate the user:
					PACKAGE.Api.ReAuthenticate(null, requestName, callback, theThis);
					return;
				}
				
				var response = req.responseXML.documentElement;
				if(PACKAGE.Api.returnHasErrors( response, true) ){
					if(errorCallback){
						errorCallback.call(theThis, req);
					}
					return;
		...
		ReAuthenticate : function (requestDoc, requestName, callback, theThis) {
			...
			var tmpWindow = new PACKAGE.dialog.ServerConnectDialog(windowTitle);
			qx.core.Init.getApplication().getRoot().add(tmpWindow);
			...
			tmpWindow.addOkEventListener( "execute", function() {
				...
				PACKAGE.Api.ConnectToServer(obj,
				function(response){ // the success callback
					var objs = PACKAGE.sqldo.LogOn.readElementChildren(response);
					...
					// resend the original request.
					if(requestDoc){
						PACKAGE.Api.SendRequest( requestDoc, requestName, callback, theThis);
					} else {
						PACKAGE.Api.LoadXMLDoc( requestName, callback, theThis );
					}
				}, null,
		...
		ConnectToServer : function ( inputObj,
			...
			PACKAGE.Api.SendRequest(requestDoc, "/logic/utils/ConnectToServer",
				completionFunction, theThis, errorFunction);
		},
		...
		ExecuteNamedQuery0: function ( queryName,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/ExecuteNamedQuery",
				completionFunction, theThis);
		},
		...
		UserProperties : function ( setList, delList,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/UserProperties",
			...
			function(response) {
				// store the return in our PACKAGE.Singleton.userProperties:
		...
		SystemProperties : function ( 
			...
			PACKAGE.Api.SendRequest(requestDoc, "/SystemProperties",
			// System properties always use the same completion function.
			// They always get stored in our global singleton so that everyone
			// can see and use them.
			function(response) {
				// store the return in our PACKAGE.Singleton.systemProperties:
				var single = PACKAGE.Singleton.getInstance();
		...
		SaveLayout : function ( layoutDoc, layoutName,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(layoutDoc, "/logic/utils/SaveLayout",
				completionFunction, theThis);	
		},
		...
		DeleteHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/DeleteHelixUser",
				completionFunction, theThis);
		},
		...
		DeleteIAFile : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/DeleteIAFile",
				completionFunction, theThis);
		},
		...
		DeleteScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/DeleteScheduleItem",
				completionFunction, theThis);
		},
		...
		GetHelixUsers : function ( 
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/GetHelixUsers",
				completionFunction, theThis);
		},
		...
		GetOneHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/GetOneHelixUser",
				completionFunction, theThis);
		},
		...
		GetSQLTest : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/GetSQLTest",
				completionFunction, theThis);
		},
		...
		InsertHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/InsertHelixUser",
				completionFunction, theThis);
		},
		...
		UpdateHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/UpdateHelixUser",
				completionFunction, theThis);
		},
		...
		InsertScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/InsertScheduleItem",
				completionFunction, theThis);
		},
		...
		UpdateScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/admin/UpdateScheduleItem",
				completionFunction, theThis);
		},
		...
		GetHelixProject : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/dev/GetHelixProject",
				completionFunction, theThis);
		},
		...
		GetHelixProjectList : function ( 
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/dev/GetHelixProjectList",
				completionFunction, theThis);
		},
		...
		SaveHelixProject : function ( inputObj,
			...
			// Now send the request to the server.
			PACKAGE.Api.SendRequest(requestDoc, "/logic/dev/SaveHelixProject",
				completionFunction, theThis);
		},
