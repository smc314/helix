...
************************************************************************ */
qx.Mixin.define("PACKAGE.StandardApp", {
	...
	members: {
		...
		*/
		setupStandardApp: function (skipSystem, customSplitPane) {
			...
			PACKAGE.Api.UserProperties({}, {}, this, function () {
		...
		loadSystemProperties: function () {
			if (this.skipSystem !== undefined && this.skipSystem === true) {
				...
				PACKAGE.Api.SystemProperties(this, function () {
		...
		getHelpMenu: function () {
			...
			aboutSPButton.addListener("execute", function () {
				...
				if (tab_page != null) { // brand new
					tab_page.setIcon("PACKAGE/icon/16x16/plain/about.png");
		...
		*/
				...
				if (props.hasOwnProperty(p)) {
					...
					if (idx1 != -1 && idx2 != -1 && idx2 > idx1) {
						...
						btn.addListener("execute", function (e) {
							...
							PACKAGE.Api.SwitchServerConnection(obj, function (response) {
								this.serverMenu.setLabel(label);
								this.addCloseConnection();
								this.loadSystemProperties();
								this.closeAllEditors();
		...
		displayServerConnectDialog: function () {
			...
			tmpWindow.addOkEventListener("execute", function () {
				...
				PACKAGE.Api.ConnectToServer(obj, function (response) {
		...
		manageServerConnectDialog : function(){
			...
			var tmpWindow = new PACKAGE.dialog.ManageServerConnections();
			tmpWindow.open();
			qx.core.Init.getApplication().getRoot().add(tmpWindow);
		...
		addCloseConnection: function () {
			...
			closeBtn.addListener("execute", function () {
				PACKAGE.Api.CloseConnection(function (response) {
					...
					this.serverMenu.setIcon("PACKAGE/icon/16x16/plain/server_delete.png");
					this.pruneTree();
				}, this);
			}, this);
		...
		addEditingTab: function (tabName) {
			...
			for (var i = 0, l = pages.length; i < l; i++) {
				if (pages[i].getLabel() === tabName ||
					...
					PACKAGE.Statics.doAlertGreen("A tab is already open with the name: " + tabName);
					return null; // it's already there
				}
			}

			// Didn't find it, so go ahead and add a new tab for it:
			var page = PACKAGE.Statics.addTabPage(this.tabView, tabName, true, true);
			return page;
		},
		...
		*/
		...
		showWelcome: function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("PACKAGE/icon/16x16/plain/house.png");
				tab_page.add(new PACKAGE.HomePage(0), { flex: 10 });
			}
		},
		...
		showUpdates : function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("PACKAGE/icon/16x16/plain/download.png");
				tab_page.add(new PACKAGE.help.CheckForUpdates(0), { flex: 10 });
			}
		},
		...
		*/
		...
		getSplitPane: function () {
			...
			var navTab = PACKAGE.Statics.addEditorSubTab( this.treeTabs, "Explorer", true, this.getTreeToolBar() );
			PACKAGE.Statics.findQXParent( navTab, qx.ui.tabview.Page ).setIcon("PACKAGE/icon/16x16/plain/folder.png" );
		...
		createTreeRMC: function () {
			...
			PACKAGE.Statics.addToRMCMenu(this.treeRMC, "Refresh Tree",
				"PACKAGE/icon/16x16/plain/refresh.png", this.fillTree, this);
			PACKAGE.Statics.addToRMCMenu(this.treeRMC, "View Selected Item",
				"PACKAGE/icon/16x16/plain/eye_blue.png", this.viewTreeSelection, this);
			PACKAGE.Statics.addToRMCMenu(this.treeRMC, "Delete Selected Item",
				"PACKAGE/icon/16x16/plain/delete2.png", this.deleteTreeSelection, this);
		...
		viewTreeSelection: function () {
			if (this.tree.getSelection().length === 0) {
				PACKAGE.Statics.doAlert("There is nothing selected in the tree.",
			...
			if (treeNode.editObject){
				...
				PACKAGE.Statics.doAlert("There is nothing to view for the current selection.",
					"Alert", this.setTreeFocus, this);
			}
		...
		getTreeToolBar: function () {
			...
			PACKAGE.Statics.addToToolbar(part0, "PACKAGE/icon/16x16/plain/refresh.png",
				"Refresh Tree", function(e) {this.doTada(); this.fillTree();}, this, this, "Refresh");

			PACKAGE.Statics.addToToolbar(part0, "PACKAGE/icon/16x16/plain/eye_blue.png",
				"View/Modify Tree Selection", this.viewTreeSelection, this, this, "View");

			PACKAGE.Statics.addToToolbar(part0, "PACKAGE/icon/16x16/plain/delete2.png",
		...
		getMainToolBar: function () {
			...
			if (props["CurrentConnection"] !== undefined) {
				...
				this.serverMenu.setIcon("PACKAGE/icon/16x16/plain/server_connection.png")
				this.addCloseConnection(this);
			}
		...
		getFileMenu: function () {
			...
			this.addMenuButton(menu, "Control+Alt+R", "Refresh List",
				"PACKAGE/icon/16x16/plain/refresh.png", null, this.fillTree);
