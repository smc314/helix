...
************************************************************************ */
/* ************************************************************************
#asset(PACKAGE/icon/basic_foundation/16x16/plain/refresh.png)
#asset(PACKAGE/icon/basic_foundation/16x16/plain/refresh.png)
#asset(PACKAGE/icon/basic_foundation/16x16/plain/edit.png)
#asset(PACKAGE/icon/basic_foundation/16x16/plain/document_new.png)
#asset(PACKAGE/icon/basic_foundation/16x16/plain/delete2.png)
#asset(PACKAGE/icon/basic_foundation/32x32/shadow/sign_warning.png)
************************************************************************ */
qx.Class.define("PACKAGE.ObjectTreeListDisplay", {
	...
	members : {
		...
		initializeToolbar : function( tb ) {
			...
			// Refresh button
			PACKAGE.Statics.addToToolbar( this.toolbar_part1, "PACKAGE/icon/basic_foundation/16x16/plain/refresh.png",
		...
		createRMCMenu : function(ev) {
			...
			// refresh
			PACKAGE.Statics.addToRMCMenu( this.rmc, "Refresh", 
				'PACKAGE/icon/basic_foundation/16x16/plain/refresh.png', this.rmcFullRefresh, this);
			
			// edit
			PACKAGE.Statics.addToRMCMenu( this.rmc, "Edit...", 
				'PACKAGE/icon/basic_foundation/16x16/plain/edit.png', this.rmcEdit, this);
			
			// Add New...
			if(this.doAddNew){
				PACKAGE.Statics.addToRMCMenu( this.rmc, "Add New...", 
					'PACKAGE/icon/basic_foundation/16x16/plain/document_new.png', this.addNew, this);
			}
			
			// Delete...
			if(this.deleteApi !== null){
				PACKAGE.Statics.addToRMCMenu( this.rmc, "Delete...", 
					'PACKAGE/icon/basic_foundation/16x16/plain/delete2.png', this.doDelete, this);
			}
		...
		rmcEdit : function(ev) {
			...
			var page = PACKAGE.StandardPage.addStandardTab( targetTabView , 
		...
		/** This will set the data object on the tree node that has the given ID.
		  ...
		  */
		setPACKAGEDataObject : function ( nodeID, dataObject ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				node.PACKAGEDataObject = dataObject;
			}
		},
		...
		/** This will get the data object on the tree node that has the given ID.
		  * The ID comes from calls to dataModel.addBranch().
		  */
		getPACKAGEDataObject : function ( nodeID ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				return node.PACKAGEDataObject;
			} else {
				return null;
			}
		},
		...
		addNew : function(ev) {
			...
			var page = PACKAGE.StandardPage.addStandardTab( this.getTargetTabView(), 
		...
		doDelete : function(ev) {
			...
			if( this.tree.getSelectionModel().isSelectionEmpty() ){
				PACKAGE.Statics.doAlert("Nothing selected to delete!");
				return;
			}

			var windowTitle = "Deleting...";
			var windowIcon = "PACKAGE/icon/basic_foundation/32x32/shadow/sign_warning.png";
			var windowLabel = "Are you sure you want to delete the selected items?";
			var tmpWindow = new PACKAGE.dialog.QuestionDialog( windowTitle, windowIcon,
				windowLabel );
	...
	destruct : function () {
		...
		PACKAGE.Statics.cleanupStandardTable( this.tree );
