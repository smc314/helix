...
************************************************************************ */
qx.Class.define("PACKAGE.OldLayoutEngine", {
	...
	statics : {
		...
		renderLayout : function ( theThis, layoutDoc, parent ) {
			...
			var obj = PACKAGE.LayoutEngine.createElementAndChildren( theThis, elem, parent );
			
			if((parent) && (obj) && (parent !== obj)){
				var flex = PACKAGE.LayoutEngine.getFlexAttr(elem);
				if(flex){
		...
		loadRenderLayout : function ( theThis, docURL, parent, callback ){
			...
			function(response){
				var obj = PACKAGE.LayoutEngine.renderLayout( theThis, response, parent);
		...
		createElementAndChildren : function (theThis, elem, parent) {
			...
			if(elem.nodeName === "VerticalBoxLayout"){
				return PACKAGE.LayoutEngine.createVBox( theThis, elem, parent );
			} else if(elem.nodeName === "HorizontalBoxLayout"){
				return PACKAGE.LayoutEngine.createHBox( theThis, elem, parent );
			} else if(elem.nodeName === "VerticalSplitPane") {
				return PACKAGE.LayoutEngine.createVerticalSplitPane( theThis, elem, parent );
			} else if(elem.nodeName === "GroupBox") {
				return PACKAGE.LayoutEngine.createGroupBox( theThis, elem, parent );
			} else if(elem.nodeName === "Tree" ) {
				return PACKAGE.LayoutEngine.createTree( theThis, elem, parent );
			} else if(elem.nodeName === "TreeVirtual" ) {
				return PACKAGE.LayoutEngine.createTreeVirtual( theThis, elem, parent );
			} else if(elem.nodeName === "RecursiveCheckBoxTree" ) {
				return PACKAGE.LayoutEngine.createRecursiveCheckBoxTree( theThis, elem, parent );
			} else if(elem.nodeName === "HorizontalSplitPane") {
				return PACKAGE.LayoutEngine.createHorizontalSplitPane( theThis, elem, parent );
			} else if(elem.nodeName === "TextArea") {
				return PACKAGE.LayoutEngine.createTextArea( theThis, elem, parent );
			} else if(elem.nodeName === "HtmlEmbed") {
				return PACKAGE.LayoutEngine.createHtmlEmbed( theThis, elem, parent );
			} else if(elem.nodeName === "IFrame") {
				return PACKAGE.LayoutEngine.createIFrame( theThis, elem, parent );
			} else if(elem.nodeName === "StandardTable") {
				return PACKAGE.LayoutEngine.createStandardTable( theThis, elem, parent );
			} else if(elem.nodeName === "StandardTreeVirtual") {
				return PACKAGE.LayoutEngine.createStandardTreeVirtual( theThis, elem, parent );
			} else if(elem.nodeName === "FieldArea") {
				return PACKAGE.LayoutEngine.createFieldArea( theThis, elem, parent );
			} else if(elem.nodeName === "Label") {
				return PACKAGE.LayoutEngine.createLabel( theThis, elem, parent );
			} else if(elem.nodeName === "Spacer") {
				return PACKAGE.LayoutEngine.createSpacer( theThis, elem, parent );
			} else if(elem.nodeName === "List") {
				return PACKAGE.LayoutEngine.createList( theThis, elem, parent );
			} else if(elem.nodeName === "Button") {
				return PACKAGE.LayoutEngine.createButton( theThis, elem, parent );
			} else if(elem.nodeName === "LayoutEngine") {
				PACKAGE.LayoutEngine.processChildren( theThis, elem, parent);
				return null;
			} else {
				return null;
			}
		...
		createVBox : function ( theThis, elem, parent ) {
			...
			PACKAGE.LayoutEngine.processAttributes( theThis, elem, obj );
			PACKAGE.LayoutEngine.processChildren( theThis, elem, obj );
			return obj;
		},
		...
		createSplitPane : function ( theThis, elem, parent, orientation ) {
			...
			if(orientation === "vertical"){
				var firstHalf = PACKAGE.Statics.xmlFindChild(elem, "VerticalSplitPaneTop" );
			} else {
				var firstHalf = PACKAGE.Statics.xmlFindChild(elem, "HorizontalSplitPaneLeft");
			}
			...
			for(var i = 0, l = firstHalf.childNodes.length; i < l; i++){
				...
				var child = PACKAGE.LayoutEngine.createElementAndChildren( theThis, node, obj );
				if(child){
					obj.add(child, flex1);
					break;
				}
			...
			if(orientation === "vertical"){
				var secondHalf = PACKAGE.Statics.xmlFindChild(elem, "VerticalSplitPaneBottom" ); 
			} else {
				var secondHalf = PACKAGE.Statics.xmlFindChild(elem, "HorizontalSplitPaneRight" ); 
			}
		...
		createVerticalSplitPane : function ( theThis, elem, parent ) {
			return PACKAGE.LayoutEngine.createSplitPane( theThis, elem, parent, "vertical" );
		},
		
		createHorizontalSplitPane : function ( theThis, elem, parent ) {
			return PACKAGE.LayoutEngine.createSplitPane( theThis, elem, parent, "horizontal" );
		},
		...
		createStandardTable : function ( theThis, elem, parent ) {
			...
			var obj = PACKAGE.Statics.createStandardTable( titles, filtered );
			PACKAGE.LayoutEngine.processAttributes( theThis, elem, obj );
		...
		createFieldArea : function ( theThis, elem, parent ) {
			...
			for(var i = 0, l = elem.childNodes.length; i < l; i ++){
				...
				if (node.nodeName === "QuadField") {
					...
					// Check for tooltips:
					PACKAGE.LayoutEngine.addToolTip(theThis, node, "tooltip1", "varName1");
					PACKAGE.LayoutEngine.addToolTip(theThis, node, "tooltip2", "varName2");
					PACKAGE.LayoutEngine.addToolTip(theThis, node, "tooltip3", "varName3");
					PACKAGE.LayoutEngine.addToolTip(theThis, node, "tooltip4", "varName4");
                    // Set read only state:
					PACKAGE.LayoutEngine.setReadOnly(theThis, node, "readOnly1", "varName1");
					PACKAGE.LayoutEngine.setReadOnly(theThis, node, "readOnly2", "varName2");
					PACKAGE.LayoutEngine.setReadOnly(theThis, node, "readOnly3", "varName3");
					PACKAGE.LayoutEngine.setReadOnly(theThis, node, "readOnly4", "varName4");
					// Check for field verification
					PACKAGE.LayoutEngine.checkFieldVerification(theThis, node);
					...
					PACKAGE.Statics.addCheckboxes(theThis, parent, first,
						node.getAttribute("varName1"),
					...
					PACKAGE.Statics.addCheckboxes2(theThis, parent, first,
					...
					var rg = PACKAGE.Statics.addRadioGroupVertical(theThis, parent, first,
						node.getAttribute("varName1"),
						node.getAttribute("label1") ,
						node.getAttribute("varName2"),
						node.getAttribute("label2") 
					...
					var rg = PACKAGE.Statics.addRadioGroupHorizontal(theThis, parent, first,
		...
		processChildren : function ( theThis, elem, obj ){
			for(var i = 0, l = elem.childNodes.length; i < l; i++){
				...
				var child = PACKAGE.LayoutEngine.createElementAndChildren( theThis, node, obj );
				if(child){
					var flex_num = PACKAGE.LayoutEngine.getFlexAttr(node);
					if(flex_num){
