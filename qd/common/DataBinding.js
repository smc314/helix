...
/** This class defines a series of static methods that make it straightforward for
  ...
  */
qx.Class.define("PACKAGE.DataBinding", {
	...
	statics : {
		...
		bindLayout : function ( theThis, dataObject, layoutDoc) {
			...
			PACKAGE.DataBinding.bindElementAndChildren( theThis, dataObject, elem);
		...
		bindElementAndChildren : function (theThis, dataObject, elem ) {
			...
			if(elem.nodeName === "VerticalBoxLayout"){
				PACKAGE.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "HorizontalBoxLayout"){
				PACKAGE.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "VerticalSplitPane") {
				PACKAGE.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "GroupBox") {
				PACKAGE.DataBinding.bindGroupBox( theThis, dataObject, elem );
			} else if(elem.nodeName === "TreeVirtual" ) {
				PACKAGE.DataBinding.bindTreeVirtual( theThis, dataObject, elem);
			} else if(elem.nodeName === "RecursiveCheckBoxTree" ) {
				PACKAGE.DataBinding.bindRecursiveCheckBoxTree( theThis, dataObject, elem );
			} else if(elem.nodeName === "HorizontalSplitPane") {
				PACKAGE.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "TextArea") {
				PACKAGE.DataBinding.bindTextArea( theThis, dataObject, elem );
			} else if(elem.nodeName === "HtmlEmbed") {
				PACKAGE.DataBinding.bindHtmlEmbed( theThis, dataObject, elem );
			} else if(elem.nodeName === "IFrame") {
				PACKAGE.DataBinding.bindIFrame( theThis, dataObject, elem );
			} else if(elem.nodeName === "StandardTable") {
				PACKAGE.DataBinding.bindStandardTable( theThis, dataObject, elem );
			} else if(elem.nodeName === "StandardTreeVirtual") {
				PACKAGE.DataBinding.bindStandardTreeVirtual( theThis, dataObject, elem );
			} else if(elem.nodeName === "FieldArea") {
				PACKAGE.DataBinding.bindFieldArea( theThis, dataObject, elem );
			} else if(elem.nodeName === "Label") {
				PACKAGE.DataBinding.bindLabel( theThis, dataObject, elem );
			} else if(elem.nodeName === "Spacer") {
				PACKAGE.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "List") {
				PACKAGE.DataBinding.bindList( theThis, dataObject, elem );
			} else if(elem.nodeName === "Button") {
				PACKAGE.DataBinding.bindButton( theThis, dataObject, elem );
			}
		},
		...
		bindGroupBox : function ( theThis, dataObject, elem) {
			...
			*/
			...
			PACKAGE.DataBinding.processChildren( theThis, dataObject, elem );
		},
		...
		bindLabel : function ( theThis, dataObject, elem) {
			...
			PACKAGE.DataBinding.processAttributes( theThis, elem, obj );
			return obj;
		},
		...
		bindField : function ( theThis, dataObject, elem, index ) {
			...
			var oc = PACKAGE.DataBinding.getObjectController( theThis, dataObject );
		...
		bindFieldArea : function ( theThis, dataObject, elem ) {
			...
			for(var i = 0, l = elem.childNodes.length; i < l; i ++){
				...
				if (node.nodeName === "QuadField") {
					PACKAGE.DataBinding.bindField( theThis, dataObject, node, 1);
					PACKAGE.DataBinding.bindField( theThis, dataObject, node, 2);
					PACKAGE.DataBinding.bindField( theThis, dataObject, node, 3);
					PACKAGE.DataBinding.bindField( theThis, dataObject, node, 4);
					...
					PACKAGE.Statics.addCheckboxes(theThis, parent, first,
                        ...
                        // Set read only state:
                    	PACKAGE.DataBinding.setReadOnly(theThis, node, "readOnly1", "varName1");
                		PACKAGE.DataBinding.setReadOnly(theThis, node, "readOnly2", "varName2");
		...
		processChildren : function ( theThis, dataObject, elem ){
			for(var i = 0, l = elem.childNodes.length; i < l; i++){
				var node = elem.childNodes[i];
				PACKAGE.DataBinding.bindElementAndChildren( theThis, dataObject, node );
			}
		}
	}
});
