...
#asset(welcome/icon/16x16/plain/server_into.png)
#asset(welcome/icon/64x64/shadow/download.png)
#asset(welcome/icon/16x16/plain/barcode.png)
#asset(welcome/icon/16x16/plain/barcode.png)
#asset(welcome/icon/16x16/plain/barcode.png)
#asset(welcome/icon/16x16/plain/barcode.png)
#asset(welcome/icon/16x16/plain/control_panel2.png)
************************************************************************ */
qx.Class.define("welcome.help.CheckForUpdates", {
	extend: welcome.ObjectEdit,
	...
	members : {
		...
		setupNamesAndFunctions : function() {
			this.selectApiCall = null; //welcome.Api.GetObjectAPICallFIXME;
		},
		...
		initializeToolbar : function( tb ) {
			...
			// Refresh button
			welcome.Statics.addToToolbarWithShortcut(sc_part, "welcome/icon/16x16/plain/refresh.png",
				this.tr("Refresh From Server"), this.doRevert, this, this, "Control-U");

			// Download button
			welcome.Statics.addToToolbarWithShortcut(sc_part, "welcome/icon/16x16/plain/download.png",
				this.tr("Download Selected Release"), this.doDownload, this, this, "Control-D");

			// Install button
			welcome.Statics.addToToolbarWithShortcut(sc_part, "welcome/icon/16x16/plain/server_into.png",
		...
		doFormLayout : function() {
			...
			this.createMainTab(welcome.Statics.addEditorSubTab(this.tabview, "General", true));
			this.createNextTab(welcome.Statics.addEditorSubTab(this.tabview, "Next Tab", false));
			// Load more tabs here
		...
		createMainTab: function(tab_page)
		{
			...
			welcome.layout.LayoutEngine.renderLayout( this,
		...
		doRevert: function(e) {
			welcome.Api.GetReleaseList( this.getObjectDetails, this );
		},
		...
		{
			var objectList = welcome.sqldo.HubManifest.readElementChildren(response);
		...
		loadDataIntoForm : function ()
		{
			...
			/* ******************************************************* */
			var props = welcome.Singleton.getInstance().getSystemProperties();
			this.overview_label1.setLabel("Hub: " + props[welcome.Singleton.HubVersion] );
			this.overview_label2.setLabel("Built: " + props[welcome.Singleton.HubBuiltOn] );
			this.overview_label3.setLabel("Mainframe: " + props[welcome.Singleton.MainframeVersion] );
			this.overview_label4.setLabel("Built: " + props[welcome.Singleton.MainframeBuiltOn] );
		...
		verifyFields : function()
		{
			...
			if( welcome.Statics.verifyAllFields(this) === -1) return -1;
		...
		doSaveToServer : function ( )
		{
			...
			if(this.isNewObject === true){
				welcome.Api.InsertObjectAPICallFIXME( this.dataObject,
				function(response){
					...
					qx.core.Init.getApplication().addNewTreeItem( "Parent Tree Node",
						...
						"welcome/icon/16x16/plain/control_panel2.png" );

				}, this);
			} else {
				welcome.Api.UpdateObjectAPICallFIXME( this.dataObject,
				function(response){
					// pick up any new information here.
		...
		doDownload : function() {
			if(this.releaseTable.getSelectionModel().isSelectionEmpty() ){
				welcome.Statics.doAlert("No Release selected to download.");
				return;
			}
			var row = this.releaseTable.getFocusedRow();
			var hubVersion = this.releaseTable.getTableModel().getValue( 0, row );
			...
			if(release !== null){
				welcome.Api.DownloadDistribution( release, function(response) {
					welcome.Statics.doAlertGreen("Release downloaded and ready to install.");
				}, this);
			}
		},
		...
		doInstall: function() {
			if(this.releaseTable.getSelectionModel().isSelectionEmpty() ){
				welcome.Statics.doAlert("No Release selected to install.");
				return;
			}
			var row = this.releaseTable.getFocusedRow();
			var hubVersion = this.releaseTable.getTableModel().getValue( 0, row );
			...
			if(release !== null){
				welcome.Api.InstallDistribution( release, function(response) {
					welcome.Statics.doAlertGreen("Release installed and Hub Server re-starting...");
				}, this);
			}
		}
	...
	********************************************************************** */
	destruct : function() {
		...
		welcome.Statics.destroyExtraObjects( this );
	}
