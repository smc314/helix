...
/* ************************************************************************
************************************************************************ */
qx.Class.define("welcome.test.TestStatics", {
	...
	statics: {
		 ...
		 * re-implement welcome.Api.SendRequest in order to accomplish this.
		 */
		SendRequest : function (requestDoc, requestName){
			...
			req.onreadystatechange = qx.lang.Function.bind(function() {
				...
				if(!req.responseXML){
					welcome.Statics.doAlert("Server is not responding.");
					return;
				}
				resp = req.responseXML.documentElement;

				if(welcome.Api.returnRequiresDB(resp)){
					welcome.Statics.doAlert("No DB Connection Available.  Log in in the real app, then run this test again.");
					return;
				}
				if(welcome.Api.returnHasErrors(resp, true, null, null)){
					return;
				}
			});
		...
		SetServerConnection : function( host, port, user, pass) {
			...
			welcome.test.TestStatics.waitForDialog( "Connect to Ivory Hub Server", function(dialog){
		...
		SetServerConnectionNoApp : function( host, port, user, pass) {
			...
			welcome.test.TestStatics.SendRequest(requestDoc, "/logic/utils/ConnectToServer");
		},
		...
		SwitchServerConnection : function( connName ){
			...
			var requestRoot = requestDoc.documentElement;
			obj.createXMLElement( requestRoot );
			welcome.test.TestStatics.SendRequest(requestDoc, "/logic/utils/SwitchServerConnection");
		},

		selectBrowserTab : function( tabName ) {
			welcome.test.TestStatics.selectTabViewPage( qx.core.Init.getApplication().treeTabs, tabName );
		},

		selectEditor : function( editorName ) {
			welcome.test.TestStatics.selectTabViewPage( qx.core.Init.getApplication().tabView, editorName );
		},

		selectEditorTab : function( editor, tabName ) {
			welcome.test.TestStatics.selectTabViewPage( editor.tabview, tabName );
		},
		...
		waitForDialog : function( title, callBack, theThis ){
			...
			var dialog = welcome.test.TestStatics.findDialog( title );
			if(dialog !== null ){
				// found it
			...
			qx.event.Timer.once( function() {
				welcome.test.TestStatics.waitForDialog( title, callBack, theThis );
			}, theThis, 100 );
		},

		waitForMessageDialog : function( title, callBack, theThis ){
			// Check to see if the dialog is present:
			var dialog = welcome.test.TestStatics.findMessageDialog( title );
			if(dialog !== null ){
				// found it
				callBack.call( theThis, dialog );
				return;
			...
			// If not, then wait for 1/2 second and try again:
			qx.event.Timer.once( function() {
				welcome.test.TestStatics.waitForMessageDialog( title, callBack, theThis );
			}, theThis, 100 );
		},
		...
		findMessageDialog : function( title ) {
			...
			for(var i = 0, l = rootChildren.length; i < l; i++){
				var child = rootChildren[i];
				if(child instanceof welcome.dialog.MessageDialog ){
					if(title !== null && title !== undefined){
						// Check the title against the window caption.
						var caption = child.getCaption();
						if(caption.indexOf( title ) !== -1){
