...
************************************************************************ */
/*
#asset(welcome/ajax-loader-16.gif)
#asset(welcome/icon/16x16/plain/server_delete.png)
#asset(welcome/icon/16x16/plain/server_connection.png)

*/
qx.Class.define("welcome.Api", {

	type : "static",

	statics : {
		/** This function will walk a welcome response XML document and look for
		...
		returnHasErrors : function ( response, doAlert, callback, theThis ) {
			if(response == null){
				if(doAlert){
					welcome.Statics.doAlert("xmlhttp response is null!");
				}
				return true;
			}
			...
			if(error_response !== null && error_response === "true"){
				if( doAlert ){
					...
					if(errors.length > 1){
						welcome.Statics.doAlert("Errors occurred during processing:<br/><br/>" +
							welcome.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					} else {
						welcome.Statics.doAlert("Error occurred during processing:<br/><br/>" +
							welcome.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					}
				}
				return true;
		...
		returnIsLong : function(response, callback, theThis, errorCallback){
			...
			var lrTask = new welcome.sqldo.LRTask();
			lrTask.readFromXML( response );
			var tmpWindow = new welcome.dialog.LRTaskDialog( lrTask, callback, errorCallback, theThis );
		...
		activateNetworkIcon : function ( theThis ){
			...
			if(theThis && theThis.toolbar && theThis.serverMenu ){
				theThis.serverMenu.setIcon("welcome/ajax-loader-16.gif");
			}
		},
		...
		deactivateNetworkIcon : function ( theThis ){
			...
			if(theThis && theThis.toolbar && theThis.serverMenu){
				if (qx.lang.String.contains(theThis.serverMenu.getLabel(), "Not Connected")) {
					theThis.serverMenu.setIcon("welcome/icon/16x16/plain/server_delete.png");
				} else {
					theThis.serverMenu.setIcon("welcome/icon/16x16/plain/server_connection.png");
				}
			}
		},
		...
		SendRequest : function( requestDoc, requestName, callback, theThis, errorCallback) {
			welcome.Api.activateNetworkIcon( theThis );
			...
			req.onreadystatechange = qx.lang.Function.bind(function() {
				...
				welcome.Api.deactivateNetworkIcon( theThis );
				if(!req.responseXML){
					if(req.status == 403){
						welcome.Statics.doAlert("You are not authorized to perform this action.");
					} else {
				...
				if(welcome.Api.CheckNullResponse( requestName, req, theThis ) === true){
					...
					if(theThis.ApiNestLevel > 5){
						welcome.Statics.doAlert("Error communicating with server.<br/><br/>Request: " +
					...
					welcome.Api.SendRequest( requestDoc, requestName, callback, theThis, errorCallback);
					return;
				}

				var response = req.responseXML.documentElement;

				if( welcome.Api.returnRequiresDB(response) ){
					// User has not yet chosen a server connection.  Prompt them to do so.
					welcome.Api.ReAuthenticate(requestDoc, requestName, callback, theThis);
					return;
				}

				if(welcome.Api.returnHasErrors( response, true, errorCallback, theThis) ){
					if(theThis){
						theThis.fireEvent( "apiCallFailed" );
					}
					return;
				...
				if(welcome.Api.returnIsLong( response, callback, theThis, errorCallback ) ) {
					return;
				}
		...
		LoadXMLDoc : function( requestName, callback, theThis, errorCallback) {
			...
			req.onreadystatechange = qx.lang.Function.bind(function() {
				...
				welcome.Api.deactivateNetworkIcon( theThis );
				if(req.responseXML === null && req.responseText.indexOf("welcome_LOG_ON") !== -1){
					// session has expired.  Re-authenticate the user:
					welcome.Api.ReAuthenticate(null, requestName, callback, theThis);
					return;
				}

				var response = req.responseXML.documentElement;
				if(welcome.Api.returnHasErrors( response, true) ){
					if(errorCallback){
						errorCallback.call(theThis, req);
					}
					return;
		...
		ReAuthenticate : function (requestDoc, requestName, callback, theThis) {
			...
			tmpWindow.addOkEventListener( "execute", function() {
				...
				welcome.Api.ConnectToServer(obj,
				function(response){ // the success callback
					...
					// resend the original request.
					if(requestDoc){
						welcome.Api.SendRequest( requestDoc, requestName, callback, theThis);
					} else {
						welcome.Api.LoadXMLDoc( requestName, callback, theThis );
					}
				}, null,
		...
		ConnectToServer : function ( inputObj,
			...
			welcome.Api.SendRequest(requestDoc, "/logic/utils/ConnectToServer",
				completionFunction, theThis, errorFunction);
		},
		...
		ExecuteNamedQuery0: function ( queryName,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/ExecuteNamedQuery",
				completionFunction, theThis);
		},
		...
		Authenticate : function ( user, pass, dbname,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/Authenticate",
				completionFunction, theThis, errorFunction);
		},
		...
		UserProperties : function ( setList, delList,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/UserProperties",
			...
			function(response) {
				// store the return in our welcome.Singleton.userProperties:
		...
		SystemProperties : function (
			...
			welcome.Api.SendRequest(requestDoc, "/SystemProperties",
			// System properties always use the same completion function.
			// They always get stored in our global singleton so that everyone
			// can see and use them.
			function(response) {
				// store the return in our welcome.Singleton.systemProperties:
				var single = welcome.Singleton.getInstance();
		...
		SaveLayout : function ( layoutDoc, layoutName,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(layoutDoc, "/logic/utils/SaveLayout",
				completionFunction, theThis);
		},
		...
		DeleteGroup : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/DeleteGroup",
				completionFunction, theThis);
		},
		...
		DeleteHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/DeleteHelixUser",
				completionFunction, theThis);
		},
		...
		DeleteIAFile : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/DeleteIAFile",
				completionFunction, theThis);
		},
		...
		DeleteScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/DeleteScheduleItem",
				completionFunction, theThis);
		},
		...
		DeleteSQLWork : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/DeleteSQLWork",
				completionFunction, theThis);
		},
		...
		GetActions : function (
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetActions",
				completionFunction, theThis);
		},
		...
		GetGroups : function (
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetGroups",
				completionFunction, theThis);
		},
		...
		GetHelixUsers : function (
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetHelixUsers",
				completionFunction, theThis);
		},
		...
		GetIAFileDetails : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetIAFileDetails",
				completionFunction, theThis);
		},
		...
		GetIAFileList : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetIAFileList",
				completionFunction, theThis);
		},
		...
		GetOneGroup : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetOneGroup",
				completionFunction, theThis);
		},
		...
		GetOneHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetOneHelixUser",
				completionFunction, theThis);
		},
		...
		GetOneScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetOneScheduleItem",
				completionFunction, theThis);
		},
		...
		GetScheduleItems : function (
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetScheduleItems",
				completionFunction, theThis);
		},
		...
		GetSQLTest : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetSQLTest",
				completionFunction, theThis);
		},
		...
		GetSQLWork : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetSQLWork",
				completionFunction, theThis);
		},
		...
		GetWorkFileList : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/GetWorkFileList",
				completionFunction, theThis);
		},
		...
		LoadSQLTestMeta : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/LoadSQLTestMeta",
				completionFunction, theThis);
		},
		...
		SaveIAFile : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/SaveIAFile",
				completionFunction, theThis);
		},
		...
		SaveSQLTest : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/SaveSQLTest",
				completionFunction, theThis);
		},
		...
		SaveSQLWork : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/SaveSQLWork",
				completionFunction, theThis);
		},
		...
		InsertGroup : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/InsertGroup",
				completionFunction, theThis);
		},
		...
		UpdateGroup : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/UpdateGroup",
				completionFunction, theThis);
		},
		...
		InsertHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/InsertHelixUser",
				completionFunction, theThis);
		},
		...
		UpdateHelixUser : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/UpdateHelixUser",
				completionFunction, theThis);
		},
		...
		InsertScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/InsertScheduleItem",
				completionFunction, theThis);
		},
		...
		UpdateScheduleItem : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/admin/UpdateScheduleItem",
				completionFunction, theThis);
		},
		...
		GetHelixProject : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/dev/GetHelixProject",
				completionFunction, theThis);
		},
		...
		GetHelixProjectList : function (
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/dev/GetHelixProjectList",
				completionFunction, theThis);
		},
		...
		SaveHelixProject : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/dev/SaveHelixProject",
				completionFunction, theThis);
		},
		...
		CleanoutLRTask : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/CleanoutLRTask",
				completionFunction, theThis);
		},
		...
		ExecuteGenericSQL : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/ExecuteGenericSQL",
				completionFunction, theThis);
		},
		...
		FindLogMessage : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/FindLogMessage",
				completionFunction, theThis);
		},
		...
		GetLogMessages : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/GetLogMessages",
				completionFunction, theThis);
		},
		...
		GetLogMessagesByRange : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/GetLogMessagesByRange",
				completionFunction, theThis);
		},
		...
		GetLogMsgCount : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/GetLogMsgCount",
				completionFunction, theThis);
		},
		...
		GetOneLRTask : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/GetOneLRTask",
				completionFunction, theThis);
		},
		...
		SaveLogMsg : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/SaveLogMsg",
				completionFunction, theThis);
		},
		...
		SwitchServerConnection : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/SwitchServerConnection",
				completionFunction, theThis);
		},
		...
		UpdateLogSettings : function ( inputObj,
			...
			// Now send the request to the server.
			welcome.Api.SendRequest(requestDoc, "/logic/util/UpdateLogSettings",
				completionFunction, theThis);
		},
