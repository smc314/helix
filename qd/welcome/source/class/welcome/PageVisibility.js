 ...
 */
qx.Class.define("welcome.PageVisibility", {
  ...
  statics : {
    ...
    getInstance : function() {
      if (!this.$$instance) {
        this.$$instance = new welcome.PageVisibility();
      }
  ...
  construct : function(document) {
    ...
    var self = this;
    // forward the event
    if (this.__apiSupported) {
      qx.bom.Event.addNativeListener(this.__doc, this.__eventName, function(e) {
        self.emit("change", e)
      });
    } else {
      if ('onfocusin' in this.__doc) {
        this.__doc.onfocusin = this.__doc.onfocusout = function(e) { self.onchange(e, self); };
      } else {
        window.onfocus = window.onblur = function(e) { self.onchange(e, self); };
      }
    }
  },
  ...
  members :
  {
    ...
    onchange : function (e, self) {
      ...
      if (e.type == 'focus' || e.type == 'focusin') {
        if (this.__doc[self.__hiddenAttr] == false && this.__doc[self.__visibilityAttr] == "visible") {return;};
        this.__doc[self.__hiddenAttr] = false;
        this.__doc[self.__visibilityAttr] = "visible";
      } else if (e.type == 'blur' || e.type == 'focusout') {
        if (this.__doc[self.__hiddenAttr] == true && this.__doc[self.__visibilityAttr] == "hidden") {return;};
        this.__doc[self.__hiddenAttr] = true;
        this.__doc[self.__visibilityAttr] = "hidden";
      }
    ...
    __checkAttributeNames : function() {
      // page visiblity api not working for 2 windows.  disable for now.
      this.__apiSupported = false;
      ...
      if (this.__hiddenAttr == null) {
        if (this.__doc["hidden"] == undefined) {
         this.__apiSupported = false;
        }
