...
************************************************************************ */
/* ************************************************************************
#asset(welcome/icon/basic_foundation/16x16/plain/refresh.png)
#asset(welcome/icon/basic_foundation/32x32/shadow/sign_warning.png)
************************************************************************ */
qx.Class.define("welcome.ObjectTreeListDisplay", {
	...
	members : {
		...
		doCreateTreeTable : function () {
			...
			this.tree.set({ height:1, minHeight:1 });
		...
		initializeToolbar : function( tb ) {
			...
			// Refresh button
			welcome.Statics.addToToolbar( this.toolbar_part1, "welcome/icon/basic_foundation/16x16/plain/refresh.png",
				"Refresh", this.rmcFullRefresh, this, this);
		...
		createRMCMenu : function(ev) {
			...
			// refresh
			welcome.Statics.addToRMCMenu( this.rmc, "Refresh",
				'welcome/icon/basic_foundation/16x16/plain/refresh.png', this.rmcFullRefresh, this);

			// edit
			welcome.Statics.addToRMCMenu( this.rmc, "Edit...",
				'welcome/icon/basic_foundation/16x16/plain/edit.png', this.rmcEdit, this);

			// Add New...
			if(this.doAddNew){
				welcome.Statics.addToRMCMenu( this.rmc, "Add New...",
					'welcome/icon/basic_foundation/16x16/plain/document_new.png', this.addNew, this);
			}

			// Delete...
			if(this.deleteApi !== null){
				welcome.Statics.addToRMCMenu( this.rmc, "Delete...",
					'welcome/icon/basic_foundation/16x16/plain/delete2.png', this.doDelete, this);
			}
		...
		rmcEdit : function(ev) {
			...
			var page = welcome.StandardPage.addStandardTab( targetTabView ,
				this.editFormPrefix + id, true);
			var edit = this.getEditForm( id );
			page.add( edit, {flex:1} );
		},
		...
		getTargetTabView : function () {
			...
			var parentTaskPage = welcome.Statics.findQXParent( this, welcome.StandardPage );
			if(parentTaskPage === null){
				targetTabView = welcome.Statics.findQXParent( this, qx.ui.tabview.TabView );
			} else {
		...
		/** This will set the data object on the tree node that has the given ID.
		  ...
		  */
		setwelcomeDataObject : function ( nodeID, dataObject ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				node.welcomeDataObject = dataObject;
			}
		},
		...
		/** This will get the data object on the tree node that has the given ID.
		  * The ID comes from calls to dataModel.addBranch().
		  */
		getwelcomeDataObject : function ( nodeID ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				return node.welcomeDataObject;
			} else {
				return null;
			}
		},
		...
		addNew : function(ev) {
			...
			var page = welcome.StandardPage.addStandardTab( this.getTargetTabView(),
				this.newFormPrefix, true);
		...
		doDelete : function(ev) {
			...
			if( this.tree.getSelectionModel().isSelectionEmpty() ){
				welcome.Statics.doAlert("Nothing selected to delete!");
				return;
			}

			var windowTitle = "Deleting...";
			var windowIcon = "welcome/icon/basic_foundation/32x32/shadow/sign_warning.png";
			var windowLabel = "Are you sure you want to delete the selected items?";
			var tmpWindow = new welcome.dialog.QuestionDialog( windowTitle, windowIcon,
				windowLabel );
	...
	destruct : function () {
		...
		welcome.Statics.cleanupStandardTable( this.tree );
