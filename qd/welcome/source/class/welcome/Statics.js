...
************************************************************************ */
/* ************************************************************************
#asset(welcome/icon/32x32/shadow/sign_warning.png)
#asset(welcome/icon/32x32/shadow/ok.png)
************************************************************************ */
qx.Class.define("welcome.Statics", {
	...
	statics: {
		...
		*/
		populateDropDown: function (dropDown, xmlDoc, elementName,
			...
			if (autoSelectItem !== undefined) {
				welcome.Statics.selectDropDownIndex(dropDown, autoSelectItem);
			}
		...
		*/
		getTextByValue: function (dropDown, value) {
			...
			var li = welcome.Statics.findListItem(dropDown.getChildControl("list"), value);
			if (li === null) {
				return "";
			} else {
				return li.getLabel();
		...
		*/
		setDropDownByValue: function (dropDown, value) {
			...
			var list = dropDown.getChildControl("list");
			var li = welcome.Statics.findListItem(list, value);
		...
		*/
		setDropDownByLabel: function (dropDown, label) {
			...
			var list = dropDown.getChildControl("list");
			var li = welcome.Statics.findListItemByLabel(list, label);
		...
		*/
		...
		getDDSelectedValue: function (dropDown) {
			if (welcome.Statics.isDropDownUnSelected(dropDown)) {
				return null;
			}
		...
		* <pre>
		var parent = welcome.Statics.findQXParent( child, qx.ui.table.Table );
		* </pre>
		*/
		findQXParent: function (child, parentType) {
			...
			if (parent instanceof parentType) {
				...
				return welcome.Statics.findQXParent(parent, parentType);
			}
		},
		...
		*/
		...
		xmlGetBase64 : function(elem) {
			var b64 = welcome.Statics.xmlGetCDATASection( elem );
			return welcome.Statics.decode64( b64 );
		},
		...
		xmlSetBase64 : function(parent, contents){
			var b64 = welcome.Statics.encode64( contents );
			welcome.Statics.xmlSetCDATASection( parent, b64 );
		},
		...
		*/
		addFieldsToForm: function (theThis, layout,
			...
			var l1 = new qx.ui.basic.Label(label1Name); welcome.Statics.trackExtraObj(theThis, l1);
			...
			welcome.Statics.trackNamedAttr(theThis, field1Name);
			welcome.Statics.setHtmlID( f1, field1Name );

			if (label2Name) {
				var l2 = new qx.ui.basic.Label(""); welcome.Statics.trackExtraObj(theThis, l2);
				l2.setWidth(5);
				hb.add(l2);

				var l3 = new qx.ui.basic.Label(label2Name); welcome.Statics.trackExtraObj(theThis, l3);
				...
				if (field2Name) {
					...
					welcome.Statics.trackNamedAttr(theThis, field2Name);
					welcome.Statics.setHtmlID( f2, field2Name );
				}
			}

			if (label3Name) {
				var l4 = new qx.ui.basic.Label(""); welcome.Statics.trackExtraObj(theThis, l4);
				l4.setWidth(5);
				hb.add(l4);

				var l5 = new qx.ui.basic.Label(label3Name); welcome.Statics.trackExtraObj(theThis, l5);
			...
			if (label4Name) {
				var l6 = new qx.ui.basic.Label(""); welcome.Statics.trackExtraObj(theThis, l6);
				l6.setWidth(5);
				hb.add(l6);

				var l7 = new qx.ui.basic.Label(label4Name); welcome.Statics.trackExtraObj(theThis, l7);
			...
			var l8 = new qx.ui.basic.Label(""); welcome.Statics.trackExtraObj(theThis, l8);
			l8.setWidth(2);
			hb.add(l8);
		...
		*/
		addTextFieldsToForm: function (theThis, layout,
			...
			welcome.Statics.trackExtraObj(theThis, l1);
			l1.setWidth(label1Size);
			hb.add(l1);
			...
			if (label3Name) {
				...
				var l3 = new qx.ui.basic.Label(label3Name); welcome.Statics.trackExtraObj(theThis, l3);
			...
			if (label4Name) {
				...
				var l3 = new qx.ui.basic.Label(label4Name); welcome.Statics.trackExtraObj(theThis, l3);
			...
			welcome.Statics.trackExtraObj(theThis, l3);
			l3.setWidth(2);
			hb.add(l3);
		...
		*/
		addTextArea: function (theThis, layout, flexHeight, labelText, textName) {
			...
			theThis[textName] = ta;
			welcome.Statics.trackNamedAttr(theThis, textName);
			welcome.Statics.setHtmlID( ta, textName );
		},
		...
		*/
		addLongLabel: function (layout, labelText, textName, theThis) {
			...
			if (textName) {
				theThis[textName] = l1;
				welcome.Statics.setHtmlID( l1, textName );
			}
		},
		...
		*/
		addBoxWithButtons: function (theThis, layout, boxName, flexHeight, flexWidth,
			...
			welcome.Statics.trackNamedAttr(theThis, boxName);
			welcome.Statics.setHtmlID( list1, boxName);
			hb.add(list1, {flex:1});
			...
			if (button1Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, button1Name);
				welcome.Statics.setHtmlID( b, button1Name);
				vb.add(b);
			}
			if (button2Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, button2Name);
				welcome.Statics.setHtmlID( b, button2Name);
				vb.add(b);
			}
			if (button3Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, button3Name);
				welcome.Statics.setHtmlID( b, button3Name);
				vb.add(b);
			}
			if (button4Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, button4Name);
				welcome.Statics.setHtmlID( b, button4Name);
				vb.add(b);
			}
		...
		*/
		addCheckboxes: function (theThis, layout, leftSize,
			...
			var tmpLabel;
			if (initialLabel) {
				tmpLabel = new qx.ui.basic.Label(initialLabel); welcome.Statics.trackExtraObj(theThis, tmpLabel);
			} else {
			...
			if (checkbox1Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, checkbox1Name);
				welcome.Statics.setHtmlID( cb1, checkbox1Name);
			}
			if (checkbox2Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, checkbox2Name);
				welcome.Statics.setHtmlID( cb2, checkbox2Name);
			}
			if (checkbox3Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, checkbox3Name);
				welcome.Statics.setHtmlID( cb3, checkbox3Name);
			}
			if (checkbox4Name) {
				...
				welcome.Statics.trackNamedAttr(theThis, checkbox4Name);
				welcome.Statics.setHtmlID( cb4, checkbox4Name);
			}
		...
		*/
		addRadioGroupVertical: function (theThis, layout, leftSize,
			...
			if (r1Name) rg.add(welcome.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r1Name, r1Label));
			if (r2Name) rg.add(welcome.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r2Name, r2Label));
			if (r3Name) rg.add(welcome.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r3Name, r3Label));
			if (r4Name) rg.add(welcome.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r4Name, r4Label));
			if (r5Name) rg.add(welcome.Statics.addRadioButtonVertical(theThis, layout,
		...
		addRadioButtonVertical: function (theThis, layout, leftSize, r1Name, r1Label
			...
			if (r1Name) {
				...
				hb.add(rb);
				theThis[r1Name] = rb;
				welcome.Statics.trackNamedAttr(theThis, r1Name);
				welcome.Statics.setHtmlID( rb, r1Name);
				layout.add(hb);
				return rb;
			} else {
				return null;
		...
		*/
		addRadioGroupHorizontal: function (theThis, layout, leftSize, groupName,
			...
			if (r2Name) {
				...
				hb.add(rb);
				theThis[r2Name] = rb;
				welcome.Statics.trackNamedAttr(theThis, r2Name);
				welcome.Statics.setHtmlID( rb, r2Name);
				rg.add(rb);
			}
			if (r3Name) {
				...
				hb.add(rb);
				theThis[r3Name] = rb;
				welcome.Statics.trackNamedAttr(theThis, r3Name);
				welcome.Statics.setHtmlID( rb, r3Name);
				rg.add(rb);
			}
			if (r4Name) {
				...
				hb.add(rb);
				theThis[r4Name] = rb;
				welcome.Statics.trackNamedAttr(theThis, r4Name);
				welcome.Statics.setHtmlID( rb, r4Name);
				rg.add(rb);
			}
			if (r5Name) {
				...
				hb.add(rb);
				theThis[r5Name] = rb;
				welcome.Statics.trackNamedAttr(theThis, r5Name);
				welcome.Statics.setHtmlID( rb, r5Name);
				rg.add(rb);
			}
			layout.add(hb);
			return rg;
		...
		*/
		createDoubleList: function (theThis, layout,
			...
			var l1 = new qx.ui.basic.Label(leftListLabel); welcome.Statics.trackExtraObj(theThis, l1);
			hb1.add(l1);
			hb1.add(new qx.ui.core.Spacer, { flex: 1 });
			var l2 = new qx.ui.core.Spacer;
			l2.setWidth(middleSize + 3);
			hb1.add(l2);
			var l3 = new qx.ui.basic.Label(rightListLabel); welcome.Statics.trackExtraObj(theThis, l3);
			hb1.add(l3);
			hb1.add(new qx.ui.core.Spacer, { flex: 1 });
			form_layout.add(hb1);
			...
			welcome.Statics.trackNamedAttr(theThis, leftListName);
			welcome.Statics.setHtmlID( list1, leftListName);
			...
			vb1.add(add_button);
			welcome.Statics.setHtmlID( add_button, "Add");
			welcome.Statics.setHtmlID( remove_button, "Remove");
			...
			welcome.Statics.trackNamedAttr(theThis, rightListName);
			welcome.Statics.setHtmlID( list2, rightListName);
			...
			add_button.addListener("execute", function () {
				...
				for (var i = 0, l = selected.length; i < l; i++) {
					...
					var match = welcome.Statics.findListItem(list1, item.getModel());
					if (match === null) {
		...
		*/
		addToToolbar: function (toolbar, icon, tooltip, action, action_this, ownerObj, label) {
			...
			welcome.Statics.setHtmlID( btn, label);
			//btn.setHeight("auto");
		...
		*/
		...
		/** Use this to destroy an array of welcome data objects
		*
		*/
		...
		*/
		addTabPage: function (tabview, name, showClose, isChecked) {
			...
			welcome.Statics.setHtmlID( tab_page.getButton(), name);
			return tab_page;
		},
		...
		*/
		...
		doAlert: function (message, title, doAfter, theThis) {
			...
			var windowIcon = "welcome/icon/32x32/shadow/sign_warning.png";
			var tmpWindow = new welcome.dialog.MessageDialog(windowTitle, windowIcon,
				message);
			if (doAfter) {
		...
		doAlertGreen: function (message, title, doAfter, theThis) {
			var windowTitle = title ? title : "Alert";
			var windowIcon = "welcome/icon/32x32/shadow/ok.png";
			var tmpWindow = new welcome.dialog.MessageDialog(windowTitle, windowIcon,
				message);
			if (doAfter) {
				tmpWindow.addOkEventListener("execute", doAfter, theThis);
		...
		/** Use this to set the welcome data object into a tree node.  This handles
		* the setting in a standard way that makes it easy to set/get the welcome
		...
		*/
		setTreeNodeDO: function (tree, nodeID, dataObject) {
			...
			realNode.welcomeDataObject = dataObject;
		},

		/** Use this to get the welcome data object from a tree node.  Use the
		* setTreeNodeDO in order to use this method properly.
		*/
		getTreeNodeDO: function (tree, nodeID) {
			var realNode = tree.getTableModel().getData()[nodeID];
			return realNode.welcomeDataObject;
		},

		/** Use this to get the welcome data object from a tree by using the row number.
		...
		*/
		...
		/** Use this to set the welcome data object into a table row.  This will work
		* with the Simple table model.
		*/
		setTableRowDO: function (table, row, dataObject) {
			table.getTableModel().getRowData(row).welcomeDataObject = dataObject;
		},

		/** Use this to get the welcome data object from a table row.  This will work
		* with the Simple table model.
		*/
		getTableRowDO: function (table, row) {
			return table.getTableModel().getRowData(row).welcomeDataObject;
		},
		...
		*/
		...
		verifyAllFields: function (theThis) {
			...
			for (var i = 0, l = theThis.fieldVerificationList.length; i < l; i++) {
				...
				var verifyResult = 0;
				if (obj.type === "string") {
					verifyResult = welcome.Statics.verifyStringField(theThis, obj);
				} else if (obj.type === "int") {
					verifyResult = welcome.Statics.verifyIntegerField(theThis, obj);
				} else {
				...
				if (verifyResult === -1) {
					...
					var our_tab = welcome.Statics.findQXParent(field, qx.ui.tabview.Page);
		...
		verifyStringField: function (theThis, verifyObj) {
			...
			if (minLen > 0 || required) {
				if (welcome.Statics.verifyTextField(field, verifyObj.label) === -1) return -1;
			}

			// Ensure that the field has a minimum length
			var val = qx.lang.String.trimRight(field.getValue());
			if (minLen > 0 && val.length < minLen) {
				welcome.Statics.doAlert(verifyObj.label + " field must be at least " + verifyObj.min + " characters.",
			...
			if (maxLen > 0 && val.length > maxLen) {
				welcome.Statics.doAlert(verifyObj.label + " field must not be longer than " + verifyObj.max + " characters.",
		...
		verifyIntegerField: function (theThis, verifyObj) {
			...
			var val = -1;
			if (required) {
				var val = welcome.Statics.verifyInteger(field, verifyObj.label);
			}
			...
			if (verifyObj.min !== undefined &&
				...
				welcome.Statics.doAlert(verifyObj.label + " field must be at least " + verifyObj.min + " in value.",
			...
			if (verifyObj.max !== undefined &&
				...
				welcome.Statics.doAlert(verifyObj.label + " field must not be larger than " + verifyObj.max + ".",
		...
		verifyTextField: function (field, name) {
			if (!field.getValue() || qx.lang.String.trimRight(field.getValue()).length === 0) {
				welcome.Statics.doAlert(name + " field is empty.  Please enter a value",
		...
		verifyDropDown: function (field, name) {
			if (welcome.Statics.isDropDownUnSelected(field)) {
				welcome.Statics.doAlert(name + " field is empty.  Please enter a value",
					name + " field is empty!", function () {
						field.focus();
					});
		...
		verifyInteger: function (field, name) {
			...
			if (welcome.Statics.verifyTextField(field, name) === -1) return -1;
			...
			if (isNaN(intValue)) {
				welcome.Statics.doAlert(name + " field has an invalid integer.  Please enter a valid integer.",
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						...
						welcome.Statics.loadOneField(theThis, dataObject, m);
					}
				}
			}
		...
		*/
		loadOneField: function (theThis, dataObject, fieldName) {
			...
			if (formField instanceof qx.ui.form.TextField ||
				...
				welcome.Statics.setDropDownByValue(formField, String(dataObject[getName]()));
		...
		pullAllFields: function (theThis, dataObject) {
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						// This is a member that ends in "Field".  Load data for it:
						welcome.Statics.pullOneField(theThis, dataObject, m);
					}
				}
			}
		...
		*/
		...
		isDevVersion: function () {
			if (welcome.Singleton.getInstance().getUserProperties()["welcomeClientVersion"].substr(0, 4) === "314.") {
				return true;
			} else {
				return false;
			}
		...
		isFreeVersion: function () {
			//var license = welcome.LicenseCode.getInstance();
		...
		areWeHomeBase: function () {
			if (window.location.hostname === "welcome.hericus.com") {
				return true;
			} else {
				return false;
			}
		...
		*/
		...
		decryptPwd: function (enc) {
			...
			for (var i = 0; i < enc.length; i++) {
				...
				// char not found
				if (done) {
					welcome.Statics.doAlert("A password contains an invalid character. The field will be cleared in the form.  If you save the record without providing a new password, the record will be saved with the cleared password.");
					return "";
				}
			}
			return temp;
		...
		*/
		addToToolbarWithShortcut: function (toolbar, icon, tooltip, action, action_this, ownerObj, shortcut, label) {
			...
			toolbar.add(button);
			welcome.Statics.setHtmlID( button, label);
		...
		isActiveEditor : function (widget) {
			var page = welcome.Statics.findQXParent(widget, qx.ui.tabview.Page);
		...
		*/
		...
		getUserFromCurConnection : function() {
			var uprops = welcome.Singleton.getInstance().getUserProperties();
