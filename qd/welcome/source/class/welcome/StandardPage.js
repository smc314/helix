...
#asset(welcome/icon/16x16/plain/window_split_ver.png)
#asset(welcome/icon/16x16/plain/window_split_hor.png)
#asset(welcome/icon/16x16/plain/window_delete.png)
************************************************************************ */
qx.Class.define("welcome.StandardPage", {
	...
	construct : function () {
		...
		this.icon = "welcome/icon/32x32/shadow/sign_warning.png";
	...
	properties : {
		 initialiwelcome : {init : false, check : "Boolean" }
	},
	...
	members : {
		...
		setupToolbarAndTabs : function( widget ) {
			...
			tabview.addListener("drop", welcome.StandardPage.handleDragDrop);
			tabview.addListener("dragover", welcome.StandardPage.handleDragOver);
		...
		rmcCloseTab : function ( e ){
			...
			var tabview = welcome.Statics.findQXParent( btn, qx.ui.tabview.TabView );
			var pageSearched = welcome.StandardPage.findTabPage( btn );
			if(pageSearched){
		...
		initializeToolbar : function( ) {
			...
			var split_vertical = new qx.ui.toolbar.Button(null, "welcome/icon/16x16/plain/window_split_ver.png");
			split_vertical.setUserData( "welcome.CurrentPage", this );
			...
			var split_horizontal = new qx.ui.toolbar.Button(null, "welcome/icon/16x16/plain/window_split_hor.png");
			split_horizontal.setUserData( "welcome.CurrentPage", this );
			...
			var split_close = new qx.ui.toolbar.Button(null, "welcome/icon/16x16/plain/window_delete.png");
			split_close.setUserData( "welcome.CurrentPage", this );
		...
		createComandCreateView : function ( callback ){
			...
			c1.addListener("execute", function(event) {
				...
				var tab_page = welcome.StandardPage.addStandardTab( tabview, event.getData().getLabel(), true);
		...
		initialTabView : function( tabView ) {
			...
			var p1_1 = welcome.StandardPage.addStandardTab( tabView, "View 1", true);
			var p1_2 = welcome.StandardPage.addStandardTab( tabView, "View 2", false);
			var p1_3 = welcome.StandardPage.addStandardTab( tabView, "View 3", false);
		...
		doSplitVertical : function( ) {
			...
			//        -> toolbar part
			//             -> toolbar button === this.
			var parentToolbar = welcome.Statics.findQXParent( this, qx.ui.toolbar.ToolBar );
			var layoutParent = parentToolbar.getLayoutParent();
			var current_page = this.getUserData("welcome.CurrentPage");
		...
		doCloseSplit : function( ) {
			...
			var our_toolbar = welcome.Statics.findQXParent( this, qx.ui.toolbar.ToolBar );
			var our_area = welcome.Statics.findQXParent(our_toolbar, qx.ui.container.Composite);
			var our_splitpane = welcome.Statics.findQXParent(our_area, qx.ui.splitpane.Pane);
			var other_area = null;
			if( welcome.Statics.getSplitFirst(our_splitpane) === our_area ){
				// we are the first area.
				other_area = welcome.Statics.getSplitSecond(our_splitpane);
			} else {
				// we are the second area.
				other_area = welcome.Statics.getSplitFirst(our_splitpane);
			}
			...
			var ourPage = null;
			if(splitpane_parent instanceof welcome.StandardPage){
				...
				ourPage = welcome.Statics.findQXParent( splitpane_parent, welcome.StandardPage );
			}
	...
	statics : {
		...
		findTabPage : function (tab_button) {
			var tabview = welcome.Statics.findQXParent( tab_button, qx.ui.tabview.TabView );
			if(!tabview){
		 ...
		 * closetab event (pointing it to welcome.StandardPage.closeTabHandler) and it will
		...
		addStandardTab : function ( tabview, tabName, set_as_checked) {
			...
			var our_spage = welcome.Statics.findQXParent( tabview, welcome.StandardPage );
			...
			tab_page.getButton().addListener("close", welcome.StandardPage.closeTabHandler, true );


			tab_page.getButton().setDraggable(true);
			tab_page.getButton().addListener("dragstart", welcome.StandardPage.handleDragStart);
			tab_page.getButton().addListener("droprequest", welcome.StandardPage.handleDropRequest);

			tab_page.setDroppable(true);
			tab_page.addListener("drop", welcome.StandardPage.handleDragDrop);
			tab_page.addListener("dragover", welcome.StandardPage.handleDragOver);
		...
		handleDragDrop : function ( e ) {
			...
			var orig_tabview = welcome.Statics.findQXParent( orig_button, qx.ui.tabview.TabView );
			var orig_page = welcome.StandardPage.findTabPage( orig_button );
			...
			if(dropping_where instanceof qx.ui.tabview.TabView){
				...
				new_tabview = welcome.Statics.findQXParent( dropping_where, qx.ui.tabview.TabView );
			}
