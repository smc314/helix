...
#asset(dev/icon/16x16/plain/server_into.png)
#asset(dev/icon/64x64/shadow/download.png)
...
************************************************************************ */
qx.Class.define("dev.help.CheckForUpdates",
{
	...
	construct : function(object_id)
	{
		...
		{
			name : "CheckForUpdates.MainPage.xml",
			doc : null
		});
		this.dynamicLayouts.push(
		{
			name : "CheckForUpdates.NextPage.xml",
			doc : null
		});
	...
	members :
	{
		...
		{
			this.selectApiCall = null;                    //dev.Api.GetObjectAPICallFIXME;
		},
		...
		initializeToolbar : function(tb)
		{
			...
			// Refresh button
			dev.Statics.addToToolbarWithShortcut(sc_part, "dev/icon/16x16/plain/refresh.png", this.tr("Refresh From Server"), this.doRevert, this, this, "Control-U");

			// Download button
			dev.Statics.addToToolbarWithShortcut(sc_part, "dev/icon/16x16/plain/download.png", this.tr("Download Selected Release"), this.doDownload, this, this, "Control-D");

			// Install button
			dev.Statics.addToToolbarWithShortcut(sc_part, "dev/icon/16x16/plain/server_into.png", this.tr("Install Selected Release"), this.doInstall, this, this, "Control-I");
			tb.add(sc_part);
			return tb;
		},
		...
		doFormLayout : function()
		{
			...
			this.createNextTab(dev.Statics.addEditorSubTab(this.tabview, "Next Tab", false));
		...
		{
			var live = this.createOverviewLayout(tab_page, "Ivory Hub Releases", "dev/icon/64x64/shadow/download.png", "Download Releases");
			this.addStatusHeading("Current Version Info");
			this.overview_label1 = this.addStatusItem("Hub: ", "dev/icon/16x16/plain/barcode.png");
			this.overview_label2 = this.addStatusItem("Built: ", "dev/icon/16x16/plain/barcode.png");
			this.overview_label3 = this.addStatusItem("Mainframe: ", "dev/icon/16x16/plain/barcode.png");
			this.overview_label4 = this.addStatusItem("Built: ", "dev/icon/16x16/plain/barcode.png");
			dev.LayoutEngine.renderLayout(this, this.getDynamicLayout("CheckForUpdates.MainPage.xml"), live);
		...
		createNextTab : function(tab_page) {
			dev.LayoutEngine.renderLayout(this, this.getDynamicLayout("CheckForUpdates.NextPage.xml"), tab_page);
		},
		...
		doRevert : function(e) {
			dev.Api.GetReleaseList(this.getObjectDetails, this);
		},
		...
		{
			var objectList = dev.sqldo.HubManifest.readElementChildren(response);
			...
			for (var i = 0, l = objectList.length; i < l; i++)
			{
				...
				data.push([obj.HubVersions[0].getFull(), obj.HubVersions[0].getDate(), obj.MainframeVersions[0].getFull(), obj.MainframeVersions[0].getDate(), Boolean(obj.getFullRelease()), Boolean(obj.getHotFix())]);
			}
		...
		loadDataIntoForm : function()
		{
			...
			/* ******************************************************* */
			var props = dev.Singleton.getInstance().getSystemProperties();
			this.overview_label1.setLabel("Hub: " + props[dev.Singleton.HubVersion]);
			this.overview_label2.setLabel("Built: " + props[dev.Singleton.HubBuiltOn]);
			this.overview_label3.setLabel("Mainframe: " + props[dev.Singleton.MainframeVersion]);
			this.overview_label4.setLabel("Built: " + props[dev.Singleton.MainframeBuiltOn]);
		...
		doSaveToServer : function()
		{
			...
			if (this.isNewObject === true) {
				dev.Api.InsertObjectAPICallFIXME(this.dataObject, function(response)
				{
					...
					qx.core.Init.getApplication().addNewTreeItem("Parent Tree Node", this.dataObject.getTName(), this.dataObject, "dev/icon/16x16/plain/control_panel2.png");
				}, this);
			} else {
				dev.Api.UpdateObjectAPICallFIXME(this.dataObject, function(response)
				{
					// pick up any new information here.
					this.loadDataIntoForm();                    // Gets all of our overview labels
					this.resetDirty();
		...
		doDownload : function()
		{
			...
			{
				dev.Statics.doAlert("No Release selected to download.");
				return;
			}
			var row = this.releaseTable.getFocusedRow();
			var hubVersion = this.releaseTable.getTableModel().getValue(0, row);
			...
			if (release !== null) {
				dev.Api.DownloadDistribution(release, function(response) {
					dev.Statics.doAlertGreen("Release downloaded and ready to install.");
				}, this);
			}
		},
		doInstall : function()
		{
			if (this.releaseTable.getSelectionModel().isSelectionEmpty())
			{
				dev.Statics.doAlert("No Release selected to install.");
				return;
			}
			var row = this.releaseTable.getFocusedRow();
			var hubVersion = this.releaseTable.getTableModel().getValue(0, row);
			...
			if (release !== null) {
				dev.Api.InstallDistribution(release, function(response) {
					dev.Statics.doAlertGreen("Release installed and Hub Server re-starting...");
				}, this);
			}
		}
	},
