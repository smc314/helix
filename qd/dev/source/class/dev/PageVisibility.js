...
qx.Class.define("dev.PageVisibility",
{
	...
	construct : function(document)
	{
		...
		if (this.__apiSupported) {
			...
			if ('onfocusin' in this.__doc) {
				this.__doc.onfocusin = this.__doc.onfocusout = function(e) {
	...
	members :
	{
		...
		onchange : function(e, self)
		{
			...
			{
				if (this.__doc[self.__hiddenAttr] == false && this.__doc[self.__visibilityAttr] == "visible") {
					return;
				};
			...
			{
				if (this.__doc[self.__hiddenAttr] == true && this.__doc[self.__visibilityAttr] == "hidden") {
					return;
				};
