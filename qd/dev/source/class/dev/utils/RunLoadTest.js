...
#asset(dev/icon/64x64/shadow/sports_car.png)
************************************************************************ */
qx.Class.define("dev.utils.RunLoadTest", {
	...
	members : {
		...
		setupNamesAndFunctions : function() {
			this.selectApiCall = dev.Api.GetOneLoadTest;
		},
		...
		initializeToolbar : function( tb ) {
			...
			dev.Statics.addToToolbar( part, "dev/icon/16x16/plain/bullet_triangle_glass_green.png",
		...
		doFormLayout : function() {
			...
			this.createNextTab(dev.Statics.addEditorSubTab(this.tabview, "Results", false));
			// Load more tabs here
		...
		createMainTab: function(tab_page)
		{
			var live = this.createOverviewLayout( tab_page, "Load Test",
				"dev/icon/64x64/shadow/sports_car.png",
		...
		initializeNewObject : function () {
			...
			this.dataObject = new dev.sqldo.LoadTest();
		...
		{
			var objectList = dev.sqldo.LoadTest.readElementChildren(response);
			if(objectList.length === 0){
				return; // Nothing to do.
			}
			this.dataObject = objectList[0];
			this.timings = dev.sqldo.LoadTestTiming.readElementChildren(response);
		...
		doRunLoadTest : function () {
			...
			dev.Api.RunLoadTest( this.dataObject, function(response) {
		...
		doSaveToServer : function ( )
		{
			...
			if(this.isNewObject === true){
				dev.Api.SaveLoadTest( this.dataObject,
				function(response){
				...
				dev.Api.UpdateLoadTest( this.dataObject,
				function(response){
					// pick up any new information here.
