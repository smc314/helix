...
#asset(dev/icon/16x16/plain/code_c.png)
#asset(dev/icon/16x16/plain/code_csharp.png)
#asset(dev/icon/16x16/plain/code_java.png)
#asset(dev/icon/64x64/shadow/scroll_run.png)
#asset(dev/icon/16x16/plain/barcode.png)
#asset(dev/icon/16x16/plain/barcode.png)
#asset(dev/templates/CTemplate.c)
#asset(dev/icon/64x64/shadow/code_c.png)
#asset(dev/templates/CSharpTemplate.cs)
#asset(dev/icon/64x64/shadow/code_csharp.png)
#asset(dev/templates/JavaTemplate.java)
#asset(dev/icon/64x64/shadow/code_java.png)
#asset(dev/icon/16x16/plain/scroll_run.png)
************************************************************************ */
qx.Class.define("dev.utils.ExecuteSQL", {
	...
	members: {
		...
		*/
		setupNamesAndFunctions: function () {
			this.selectApiCall = dev.Api.GetSQLWork;
		},
		...
		*/
		initializeToolbar: function (tb) {
			...
			dev.Statics.addToToolbarWithShortcut(part, "dev/icon/16x16/plain/bullet_triangle_glass_green.png",
				"Run SQL Statement", this.runSQL, this, this, null, "Run SQL");

			part.addSeparator();

			dev.Statics.addToToolbarWithShortcut(part, "dev/icon/16x16/plain/code_c.png",
				"Generate C Code For SQL", this.genCCode, this, this, null, "C Code");

			dev.Statics.addToToolbarWithShortcut(part, "dev/icon/16x16/plain/code_csharp.png",
				"Generate C# Code For SQL", this.genCSharpCode, this, this, null, "C# Code");

			dev.Statics.addToToolbarWithShortcut(part, "dev/icon/16x16/plain/code_java.png",
		...
		*/
		doFormLayout: function () {
			...
			this.resultsTabPage = this.createResultTab(dev.Statics.addEditorSubTab(this.tabview, "Results", false));
			this.createMetaTab(dev.Statics.addEditorSubTab(this.tabview, "Results Metadata", false));
			// Load more tabs here
		...
		*/
		createMainTab: function (tab_page) {
			var live = this.createOverviewLayout( tab_page, "SQL Workbench",
				"dev/icon/64x64/shadow/scroll_run.png",
			...
			dev.Statics.addToDropDown( this.HostDBField, "Legacy Data Server", "lds" );
			dev.Statics.addToDropDown( this.HostDBField, "DB2/zOS", "db2" );
			dev.Statics.addToDropDown( this.HostDBField, "SQL/DS or DB2/zVSE", "sds" );
			dev.Statics.addToDropDown( this.HostDBField, "Datacom", "dso" );
			dev.Statics.addToDropDown( this.HostDBField, "Director-Config", "Director-Config" );
			//dev.Statics.addToDropDown( this.HostDBField, "Director-Logs", "Director-Logs" );
			dev.Statics.addToDropDown( this.HostDBField, "Director-MFLogs", "Director-MFLogs" );
		...
		*/
		...
		getCurrentSQL : function() {
			...
			if (!sql) {
				dev.Statics.doAlert("No SQL statement found. Enter a valid SQL statement.")
				return null;
			}

			sql = qx.lang.String.trimRight(sql);
			if (sql.length == 0) {
				dev.Statics.doAlert("The SQL statement contains only blanks.  Enter a valid SQL statement.")
				return null;
			}
		...
		*/
		runSQL: function (e) {
			...
			//if (!dev.Statics.isActiveEditor(this)) return;
			...
			sqlDO.setHostDB(dev.Statics.getDDSelectedValue(this.HostDBField));
			...
			dev.Api.ExecuteGenericSQL(sqlDO,
			function (response) {
				...
				dev.Statics.cleanupStandardTable(this.resultsTable);
				this.resultsTable = null;

				this.resultsTable = dev.Statics.createStandardTable(colNames, resultsRowData);
					...
					if (node.nodeName === "Row") {
						...
						for (var j = 0, k = node.childNodes.length; j < k; j++) {
							...
							if (node2.nodeName === "Column") {
								colData.push(
									dev.Statics.xmlGetBase64(
										dev.Statics.xmlFindChild(node2, "value")
									)
								);
							}
						}
				...
				var our_tab = dev.Statics.findQXParent(this.resultsTable, qx.ui.tabview.Page);
		...
		genCCode : function() {
			...
			dev.Api.LoadPlainDoc( qx.util.ResourceManager.getInstance().toUri("dev/templates/CTemplate.c"),
			function(response) {
				...
				var dialog = new dev.dialog.CodeDisplayDialog(winTitle, winIcon, winLabel);
				dialog.open();
		...
		genCSharpCode : function() {
			...
			dev.Api.LoadPlainDoc( qx.util.ResourceManager.getInstance().toUri("dev/templates/CSharpTemplate.cs"),
			function(response) {
				...
				var winTitle = "C# Code for Executing SQL";
				var winLabel = "C# Code to execute the selected SQL Statements<br/>Copy this code and paste it into your favorite development environment.";
				var winIcon = "dev/icon/64x64/shadow/code_csharp.png";
				var dialog = new dev.dialog.CodeDisplayDialog(winTitle, winIcon, winLabel);
				dialog.open();
				dialog.setText( qx.bom.Template.render( response, templateParms ) );
			}, this);
		...
		genJavaCode : function() {
			...
			dev.Api.LoadPlainDoc( qx.util.ResourceManager.getInstance().toUri("dev/templates/JavaTemplate.java"),
			function(response) {
				...
				var winIcon = "dev/icon/64x64/shadow/code_java.png";
				var dialog = new dev.dialog.CodeDisplayDialog(winTitle, winIcon, winLabel);
				dialog.open();
				dialog.setText( qx.bom.Template.render( response, templateParms ) );
			}, this);
		...
		*/
		initializeNewObject: function () {
			...
			this.dataObject = new dev.sqldo.SQLWork;
		...
		*/
		getObjectDetails: function (response) {
			var objectList = dev.sqldo.SQLWork.readElementChildren(response);
		...
		*/
		doSaveToServer: function () {
			...
			if(this.isNewObject === true){
				dev.Api.SaveSQLWork( this.dataObject,
				function(response){
					...
					qx.core.Init.getApplication().addNewTreeItem( "SQL Workbench Sessions",
						obj.getFileName(), obj,
						"dev/icon/16x16/plain/scroll_run.png",
						dev.utils.SqlWorkTreeNodes.editObject);
