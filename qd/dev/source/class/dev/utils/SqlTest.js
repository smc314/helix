...
#asset(dev/icon/64x64/shadow/scroll.png)
...
************************************************************************ */
qx.Class.define("dev.utils.SqlTest", {
	...
	members : {
		...
		setupNamesAndFunctions : function() {
			this.selectApiCall = dev.Api.GetSQLTest;
		},
		...
		initializeToolbar : function( tb ) {
			...
			//dev.Statics.addToToolbar( part, "dev/icon/16x16/plain/scroll2_add.png",
		...
		createMainTab: function(tab_page)
		{
			var live = this.createOverviewLayout( tab_page, "SQL Test",
				"dev/icon/64x64/shadow/scroll.png",
		...
		createNextTab: function(tab_page)
		{
			...
			this.createSqlMetaTab( dev.Statics.addEditorSubTab( this.sqlTabView, "Metadata", false));
			this.createSqlResultsTab( dev.Statics.addEditorSubTab( this.sqlTabView, "Saved Results", false));
		...
		stmtChange: function(e) {
			if(this.currentStmt !== null){
				...
				dev.Statics.pullAllFields(this, this.currentStmt);
			}
			...
			if(row !== null){
				...
				dev.Statics.loadAllFields(this, this.currentStmt);
		...
		loadResults: function() {
			...
			this.stmtResultsTable = dev.Statics.createStandardTable( colNames, null );
			...
			dev.Statics.cleanupStandardTable( this.stmtResultsCheckTable );
			this.stmtResultsCheckTable = null;

			this.stmtResultsCheckTable = dev.Statics.createStandardTable( colNames, null );
		...
		initializeNewObject : function () {
			...
			this.dataObject = new dev.sqldo.SQLTests;
		...
		{
			var objectList = dev.sqldo.SQLTests.readElementChildren(response);
		...
		addSqlStatement : function() {
			...
			var stmt = new dev.sqldo.SQLStmt;
		...
		clearStatement: function() {
			var stmt = new dev.sqldo.SQLStmt;
			dev.Statics.loadAllFields(this, stmt);
		},
		...
		doSaveToServer : function ( )
		{
			...
			if(this.isNewObject === true){
				dev.Api.SaveSQLTest( this.dataObject,
				function(response){
					...
					var treeObj = new dev.sqldo.IAFile;
					...
					qx.core.Init.getApplication().addNewTreeItem( "SQL Tests",
						...
						"dev/icon/16x16/plain/scroll.png" );
		...
		loadSqlStatement: function() {
			...
			dev.Api.LoadSQLTestMeta( this.dataObject,
			function(response) {
		...
		checkSqlStatement: function() {
			...
			// Now send our sql test group to the server for processing:
			dev.Api.CheckSQLTest( this.dataObject,
			function(response) {
				this.getObjectDetails(response);
			}, this);
