...
qx.Class.define("dev.LayoutEngine",
{
	...
	statics :
	{
		...
		loadRenderLayout : function(theThis, docURL, parent, callback) {
			dev.Api.LoadXMLDoc(docURL, function(response)
			{
		...
		{
			var obj = new qx.ui.embed.Iframe().set( {
				decorator : null
			});
			dev.LayoutEngine.processAttributes(theThis, elem, obj);
		...
		createStandardTable : function(theThis, elem, parent)
		{
			...
			if(elem.getAttribute("buttons") !== "none"){
				// If the user has given us add/remove/move-up/move-down buttons, then add
				// the extra layout details here automatically.
				var addBtn = null;
				var delBtn = null;
				var upBtn = null;
				var dnBtn = null;
				if(elem.getAttribute("add") === "true"){
					addBtn = new qx.ui.form.Button( "", "dev/icon/16x16/plain/scroll2_add.png" );
					addBtn.setToolTipText( "Add a new row to this table." );
				}
				if(elem.getAttribute("remove") === "true"){
					delBtn = new qx.ui.form.Button( "", "dev/icon/16x16/plain/delete2.png" );
					delBtn.setToolTipText( "Remove the selected rows from this table." );
				}
				if(elem.getAttribute("moveup") === "true"){
					upBtn = new qx.ui.form.Button( "", "dev/icon/16x16/plain/navigate_up2.png" );
					upBtn.setToolTipText( "Move selected rows up in the table." );
				}
				if(elem.getAttribute("movedown") === "true"){
					dnBtn = new qx.ui.form.Button( "", "dev/icon/16x16/plain/navigate_down2.png" );
					dnBtn.setToolTipText( "Move selected rows down in the table." );
				}
				if(addBtn !== null || delBtn !== null || upBtn !== null || dnBtn !== null ){
					
					var hbox = new qx.ui.container.Composite(new qx.ui.layout.HBox);
					var vbox = new qx.ui.container.Composite(new qx.ui.layout.VBox);
					vbox.setPaddingTop( 20 );
					if(addBtn !== null){
						vbox.add( addBtn );
					}
					if(delBtn !== null){
					...
					if( elem.getAttribute("buttons") === "left" ){
						hbox.add( vbox );
						hbox.add( obj, {flex: 1} );
					} else if(elem.getAttribute("buttons") === "right"){
						hbox.add( obj, {flex: 1} );
						hbox.add( vbox );
					}

					obj = hbox; // ensure that we return the container, and not the table directly.
				}
			}
		...
		addToolTip : function(theThis, node, tipAttr, varAttr)
		{
			...
			if (tipText)
			{
				...
				if (tipText.indexOf("<br") != -1 || tipText.indexOf("<p>") != -1)
				{
		...
		createFieldArea : function(theThis, elem, parent)
		{
			...
			for (var i = 0, l = elem.childNodes.length; i < l; i++)
			{
				...
				{
					dev.Statics.addFieldsToForm(theThis, parent, node.getAttribute("label1"), first, node.getAttribute("varName1"), node.getAttribute("type1"), node.getAttribute("label2"), second, node.getAttribute("varName2"), node.getAttribute("type2"), node.getAttribute("label3"), first, node.getAttribute("varName3"), node.getAttribute("type3"));
				...
				{
					dev.Statics.addFieldsToForm(theThis, parent, node.getAttribute("label1"), first, node.getAttribute("varName1"), node.getAttribute("type1"), node.getAttribute("label2"), second, node.getAttribute("varName2"), node.getAttribute("type2"));
				...
				{
					dev.Statics.addFieldsToForm(theThis, parent, node.getAttribute("label1"), first, node.getAttribute("varName1"), node.getAttribute("type1"));
					...
					{
						theThis[node.getAttribute("varName1")].setMaxHeight(Number(node.getAttribute("height1")));
						theThis[node.getAttribute("varName1")].setHeight(Number(node.getAttribute("height1")));
					}
				...
				{
					dev.Statics.addCheckboxes(theThis, parent, first, node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"));
				...
				{
					dev.Statics.addCheckboxes2(theThis, parent, first, Number(node.getAttribute("width1")), Number(node.getAttribute("width2")), 0, 0, node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"));
				...
				{
					dev.Statics.addCheckboxes2(theThis, parent, first, Number(node.getAttribute("width1")), Number(node.getAttribute("width2")), Number(node.getAttribute("width3")), Number(node.getAttribute("width4")), node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"), node.getAttribute("varName3"), node.getAttribute("label3"), node.getAttribute("varName4"), node.getAttribute("label4"));
				...
				{
					dev.Statics.addCheckboxes(theThis, parent, first, node.getAttribute("varName1"), node.getAttribute("label1"));

					// Set read only state:
					dev.LayoutEngine.setReadOnly(theThis, node, "readOnly1", "varName1");
				} else if (node.nodeName === "RadioGroup2")
				{
					var rg = dev.Statics.addRadioGroupVertical(theThis, parent, first, node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"));
				...
				{
					var rg = dev.Statics.addRadioGroupVertical(theThis, parent, first, node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"), node.getAttribute("varName3"), node.getAttribute("label3"));
					theThis[node.getAttribute("groupName")] = rg;
				...
				{
					var rg = dev.Statics.addRadioGroupVertical(theThis, parent, first, node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"), node.getAttribute("varName3"), node.getAttribute("label3"), node.getAttribute("varName4"), node.getAttribute("label4"));
					theThis[node.getAttribute("groupName")] = rg;
				...
				{
					var rg = dev.Statics.addRadioGroupHorizontal(theThis, parent, first, null,          // groupName is not used
					node.getAttribute("labelPrefix"), node.getAttribute("varName1"), node.getAttribute("label1"), node.getAttribute("varName2"), node.getAttribute("label2"), node.getAttribute("varName3"), node.getAttribute("label3"), node.getAttribute("varName4"), node.getAttribute("label4"));
					theThis[node.getAttribute("groupName")] = rg;
					...
					dev.Statics.addLongLabel(parent, node.getAttribute("label"), node.getAttribute("varName"), theThis);
				} else if (node.nodeName === "LongLabelRich") {
					dev.Statics.addLongLabelRich(parent, node.getAttribute("label"), node.getAttribute("varName"), theThis);
				}
			}
