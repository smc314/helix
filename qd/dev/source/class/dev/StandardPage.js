...
#asset(dev/icon/16x16/plain/window_split_ver.png)
#asset(dev/icon/16x16/plain/window_split_hor.png)
#asset(dev/icon/16x16/plain/window_delete.png)
************************************************************************ */
qx.Class.define("dev.StandardPage",
{
	...
	construct : function()
	{
		...
		this.icon = "dev/icon/32x32/shadow/sign_warning.png";
	...
	members :
	{
		...
		setupToolbarAndTabs : function(widget)
		{
			...
			tabview.addListener("drop", dev.StandardPage.handleDragDrop);
			tabview.addListener("dragover", dev.StandardPage.handleDragOver);
			widget.add(tabview, {
				flex : 10
			});
			return tabview;
		...
		rmcCloseTab : function(e)
		{
			...
			var tabview = dev.Statics.findQXParent(btn, qx.ui.tabview.TabView);
			var pageSearched = dev.StandardPage.findTabPage(btn);
			if (pageSearched)
			{
		...
		initializeToolbar : function()
		{
			...
			var split_vertical = new qx.ui.toolbar.Button(null, "dev/icon/16x16/plain/window_split_ver.png");
			split_vertical.setUserData("dev.CurrentPage", this);
			...
			var split_horizontal = new qx.ui.toolbar.Button(null, "dev/icon/16x16/plain/window_split_hor.png");
			split_horizontal.setUserData("dev.CurrentPage", this);
			...
			var split_close = new qx.ui.toolbar.Button(null, "dev/icon/16x16/plain/window_delete.png");
			split_close.setUserData("dev.CurrentPage", this);
		...
		populateNewMenu : function(nmenu, nbtn)
		{
			...
			for (var label in menus) {
				if ((label !== "MenuLabelIcon") && (menus.hasOwnProperty(label)))
				{
					var obj = menus[label];
					this.addViewMenuItem(nmenu, nbtn, label, obj.icon, obj.callback);
				}
			}
		},
		...
		populateViewMenu : function(vmenu, vbtn)
		{
			...
			for (var label in menus) {
				...
				{
					var obj = menus[label];
					this.addViewMenuItem(vmenu, vbtn, label, obj.icon, obj.callback);
				}
			}
		},
				...
				if (nmenus.hasOwnProperty(name)) {
					if (name !== current_name)
					{
						...
						for (var label in menus) {
							...
							{
								var obj = menus[label];
								this.addViewMenuItem(sub, nbtn, label, obj.icon, obj.callback);
							}
						}
				...
				if (vmenus.hasOwnProperty(name)) {
					if (name !== current_name)
					{
						...
						for (var label in menus) {
							...
							{
								var obj = menus[label];
								this.addViewMenuItem(sub, vbtn, label, obj.icon, obj.callback);
							}
						}
		...
		createComandCreateView : function(callback)
		{
			...
			c1.addListener("execute", function(event)
			{
				...
				var parentToolbar = dev.Statics.findQXParent(menu_Button, qx.ui.toolbar.ToolBar);
				...
				var tab_page = dev.StandardPage.addStandardTab(tabview, event.getData().getLabel(), true);
				var f1 = callback();
		...
		{
			var p1_1 = dev.StandardPage.addStandardTab(tabView, "View 1", true);
			var p1_2 = dev.StandardPage.addStandardTab(tabView, "View 2", false);
			var p1_3 = dev.StandardPage.addStandardTab(tabView, "View 3", false);
		...
		doSplitVertical : function()
		{
			...
			//             -> toolbar button === this.
			var parentToolbar = dev.Statics.findQXParent(this, qx.ui.toolbar.ToolBar);
			var layoutParent = parentToolbar.getLayoutParent();
			var current_page = this.getUserData("dev.CurrentPage");
		...
		doCloseSplit : function()
		{
			...
			var our_toolbar = dev.Statics.findQXParent(this, qx.ui.toolbar.ToolBar);
			var our_area = dev.Statics.findQXParent(our_toolbar, qx.ui.container.Composite);
			var our_splitpane = dev.Statics.findQXParent(our_area, qx.ui.splitpane.Pane);
			var other_area = null;
			if (dev.Statics.getSplitFirst(our_splitpane) === our_area) {
				// we are the first area.
				other_area = dev.Statics.getSplitSecond(our_splitpane);
			} else {
				// we are the second area.
				other_area = dev.Statics.getSplitFirst(our_splitpane);
			}
			...
			var ourPage = null;
			if (splitpane_parent instanceof dev.StandardPage)
			{
			...
			{
				ourPage = dev.Statics.findQXParent(splitpane_parent, dev.StandardPage);
			}
	...
	statics :
	{
		...
		{
			var tabview = dev.Statics.findQXParent(tab_button, qx.ui.tabview.TabView);
			if (!tabview)
			{
		 ...
		 * closetab event (pointing it to dev.StandardPage.closeTabHandler) and it will
		...
		{
			var our_spage = dev.Statics.findQXParent(tabview, dev.StandardPage);
			...
			tab_page.getButton().addListener("close", dev.StandardPage.closeTabHandler, true);
			tab_page.getButton().setDraggable(true);
			tab_page.getButton().addListener("dragstart", dev.StandardPage.handleDragStart);
			tab_page.getButton().addListener("droprequest", dev.StandardPage.handleDropRequest);
			tab_page.setDroppable(true);
			tab_page.addListener("drop", dev.StandardPage.handleDragDrop);
			tab_page.addListener("dragover", dev.StandardPage.handleDragOver);
			if (set_as_checked) {
				tab_page.getButton().setValue(true);
			}
		...
		handleDragDrop : function(e)
		{
			...
			var orig_tabview = dev.Statics.findQXParent(orig_button, qx.ui.tabview.TabView);
			var orig_page = dev.StandardPage.findTabPage(orig_button);
			...
			if (dropping_where instanceof qx.ui.tabview.TabView) {
				...
				new_tabview = dev.Statics.findQXParent(dropping_where, qx.ui.tabview.TabView);
			}
