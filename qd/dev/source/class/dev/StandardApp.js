...
/* ************************************************************************
#asset(dev/icon/16x16/plain/home.png)
#asset(dev/icon/16x16/plain/control_panel.png)
#asset(dev/icon/16x16/plain/compass.png)
#asset(dev/icon/16x16/plain/house.png)
#asset(dev/icon/16x16/plain/help2.png)
#asset(dev/icon/16x16/plain/question_and_answer.png)
#asset(dev/icon/16x16/plain/book_blue_open2.png)
...
#asset(dev/icon/16x16/plain/server_edit.png)
#asset(dev/icon/16x16/plain/server_connection.png)
#asset(dev/icon/16x16/plain/server_connection.png)
...
#asset(dev/icon/16x16/plain/house.png)
#asset(dev/icon/16x16/plain/download.png)
#asset(dev/helix-about-background.png)
#asset(dev/icon/16x16/plain/folder.png)
#asset(dev/icon/16x16/plain/refresh.png)
#asset(dev/icon/16x16/plain/eye_blue.png)
...
#asset(dev/icon/16x16/plain/refresh.png)
#asset(dev/icon/16x16/plain/eye_blue.png)
#asset(dev/icon/16x16/plain/signpost.png)
#asset(dev/icon/16x16/plain/carabiner.png)
#asset(dev/icon/16x16/plain/server_delete.png)
#asset(dev/icon/16x16/plain/server_connection.png)
#asset(dev/icon/16x16/plain/refresh.png)
************************************************************************ */
qx.Mixin.define("dev.StandardApp",
{
	properties : {
	...
	members :
	{
		...
		*/
		setupStandardApp : function(skipSystem, customSplitPane)
		{
			...
			}, this, function()
			{
				...
				if (customSplitPane) {
					doc.add(customSplitPane.call(this),
					{
						left : 0,
						top : 32,
						right : 0,
			...
			var self = this;
			dev.PageVisibility.getInstance().on("change", function() {
				if (dev.PageVisibility.getInstance().isHidden())
				{
				...
				} else
				{
					...
					}, self, function()
					{
						...
						if (label != props["CurrentConnection"])
						{
							...
							dev.Api.SwitchServerConnection(obj, function(response)
							{
		...
		*/
		getAppsMenu : function()
		{
			...
			var welcome = new qx.ui.menu.Button("Welcome To Helix Development", "dev/icon/16x16/plain/home.png");
			var admin = new qx.ui.menu.Button("Helix Administration", "dev/icon/16x16/plain/control_panel.png");
			var dev = new qx.ui.menu.Button("Helix Development", "dev/icon/16x16/plain/compass.png");
		...
		*/
		...
		{
			var menu = new qx.ui.menu.Menu;
			var welcomeButton = new qx.ui.menu.Button("View Welcome Page", "dev/icon/16x16/plain/house.png");
			var helpButton = new qx.ui.menu.Button("View Help", "dev/icon/16x16/plain/help2.png");
			var supportButton = new qx.ui.menu.Button("Helix Q&A", "dev/icon/16x16/plain/question_and_answer.png");
			var appGuidesButton = new qx.ui.menu.Button("Helix Development Guide", "dev/icon/16x16/plain/book_blue_open2.png");
			var aboutSPButton = new qx.ui.menu.Button("About Helix", "dev/icon/16x16/plain/about.png");
			var updatesButton = new qx.ui.menu.Button("Check For Updates...", "dev/icon/16x16/plain/download.png");
			...
			aboutSPButton.addListener("execute", function()
			{
				...
				if (tab_page != null)
				{                                            // brand new
					tab_page.setIcon("dev/icon/16x16/plain/about.png");
		...
		*/
		...
		{
			var menu = new qx.ui.menu.Menu;
			var btnNew = new qx.ui.menu.Button("New Connection...", "dev/icon/16x16/plain/server_connection.png");
			btnNew.addListener("execute", function() {
				this.displayServerConnectDialog();
			}, this);
			menu.add(btnNew);
			var btnMng = new qx.ui.menu.Button("Manage Connections", "dev/icon/16x16/plain/server_edit.png");
			...
			for (var p in props) {
				if (props.hasOwnProperty(p))
				{
					...
					if (idx1 != -1 && idx2 != -1 && idx2 > idx1)
					{
						...
						var btn = new qx.ui.menu.Button(p, "dev/icon/16x16/plain/server_connection.png");
		...
		displayServerConnectDialog : function()
		{
			...
			tmpWindow.addOkEventListener("execute", function()
			{
				...
				dev.Api.ConnectToServer(obj, function(response)
				{
		...
		{
			var tmpWindow = new dev.dialog.ManageServerConnections();
			tmpWindow.open();
			qx.core.Init.getApplication().getRoot().add(tmpWindow);
		},
		...
		*/
		addToServerMenu : function(newLabel)
		{
			...
			for (var i = 0, l = children.length; i < l; i++) {
				if (!(children[i] instanceof qx.ui.menu.Separator) && children[i].getLabel() == newLabel) {
			...
			var btn = new qx.ui.menu.Button(newLabel, "dev/icon/16x16/plain/server_connection.png");
		...
		removeFromServerMenu : function(menuLabel)
		{
			...
			var children = this.serverMenu.getMenu().getChildren();
			for (var i = 0, l = children.length; i < l; i++) {
				if (!(children[i] instanceof qx.ui.menu.Separator) && children[i].getLabel() == menuLabel)
				{
		...
		addCloseConnection : function()
		{
			...
			var children = this.serverMenu.getMenu().getChildren();
			for (var i = 0, l = children.length; i < l; i++) {
				if (!(children[i] instanceof qx.ui.menu.Separator) && children[i].getLabel() === "Close Current Connection") {
					// The label already exists.  Bail out.
					return;
				}
			}
			...
			var closeBtn = new qx.ui.menu.Button("Close Current Connection", "dev/icon/16x16/plain/server_delete.png");
			closeBtn.addListener("execute", function() {
				dev.Api.CloseConnection(function(response)
				{
					this.serverMenu.setLabel("Server: Not Connected");
					this.serverMenu.setIcon("dev/icon/16x16/plain/server_delete.png");
					this.pruneTree();
				}, this);
			}, this);
		...
		*/
		addEditingTab : function(tabName)
		{
			...
			for (var i = 0, l = pages.length; i < l; i++) {
				if (pages[i].getLabel() === tabName || pages[i].getLabel() === "*" + tabName)
				{
					pages[i].getButton().setValue(true);        // bring it to the front.
					dev.Statics.doAlertGreen("A tab is already open with the name: " + tabName);
					return null;                                // it's already there
				}
			}

			// Didn't find it, so go ahead and add a new tab for it:
			var page = dev.Statics.addTabPage(this.tabView, tabName, true, true);
			return page;
		},
		...
		*/
		...
		showHelp : function()
		{
			...
			if (tab_page != null)
			{                                             // brand new
				tab_page.setIcon("dev/icon/16x16/plain/help2.png");
				var iframe = new qx.ui.embed.Iframe().set( {
					source : "/dev/NetHelp/dev.htm"
				});
				tab_page.add(iframe, {
					flex : 10
				});
		...
		showDocs : function()
		{
			...
			if (tab_page != null)
			{                                             // brand new
				tab_page.setIcon("dev/icon/16x16/plain/book_blue_open2.png");
				var iframe = new qx.ui.embed.Iframe().set( {
		...
		showQA : function()
		{
			...
			if (tab_page != null)
			{                                             // brand new
				tab_page.setIcon("dev/icon/16x16/plain/question_and_answer.png");
				var iframe = new qx.ui.embed.Iframe().set( {
		...
		showWelcome : function()
		{
			...
			if (tab_page != null)
			{                                             // brand new
				tab_page.setIcon("dev/icon/16x16/plain/house.png");
				tab_page.add(new dev.HomePage(0), {
					flex : 10
				});
			}
		},
		showUpdates : function()
		{
			...
			if (tab_page != null)
			{                                             // brand new
				tab_page.setIcon("dev/icon/16x16/plain/download.png");
				tab_page.add(new dev.help.CheckForUpdates(0), {
					flex : 10
				});
			}
		},
		...
		*/
		commonAbout : function()
		{
			...
			var windowIcon = null;
			var windowText = "<br/><br/><br/><br/><br/><br/><p style=\"text-shadow: black 1px 1px 1px; font-family: QuicksandBold,sans-serif;weight:bold;color:white;\">" + this.ApplicationName + "<br/><br/>" + "Helix Version: " + this.HelixVersion + "<br/>" + "<br/>" + "Copyright &copy; 1997 - " + d.getFullYear() + " Hericus Software, Inc.  All Rights Reserved." + "</p>";
			var decorator = new qx.ui.decoration.Background();
			decorator.setBackgroundImage("dev/helix-about-background.png");
			var tmpWindow = new dev.dialog.MessageDialog(windowTitle, windowIcon, windowText);
		...
		getSplitPane : function()
		{
			...
			var navTab = dev.Statics.addEditorSubTab(this.treeTabs, "Explorer", true, this.getTreeToolBar());
			dev.Statics.findQXParent(navTab, qx.ui.tabview.Page).setIcon("dev/icon/16x16/plain/folder.png");
			navTab.add(this.tree, {
				flex : 1
			});
			this.pane.add(this.container1, 0);
			this.pane.add(this.container2, 1);
			this.tabView = new qx.ui.tabview.TabView;
			this.container2.add(this.tabView, {
				flex : 10
			});
			return this.pane;
		},
		createTreeRMC : function()
		{
			...
			dev.Statics.addToRMCMenu(this.treeRMC, "Refresh Tree", "dev/icon/16x16/plain/refresh.png", this.fillTree, this);
			dev.Statics.addToRMCMenu(this.treeRMC, "View Selected Item", "dev/icon/16x16/plain/eye_blue.png", this.viewTreeSelection, this);
			dev.Statics.addToRMCMenu(this.treeRMC, "Delete Selected Item", "dev/icon/16x16/plain/delete2.png", this.deleteTreeSelection, this);
			return this.treeRMC;
		},
		...
		viewTreeSelection : function()
		{
			...
			{
				dev.Statics.doAlert("There is nothing selected in the tree.", "Alert", this.setTreeFocus, this);
			...
			if (treeNode.editObject) {
				...
				dev.Statics.doAlert("There is nothing to view for the current selection.", "Alert", this.setTreeFocus, this);
			}
		...
		getTreeToolBar : function()
		{
			...
			toolbar.add(part0);
			dev.Statics.addToToolbar(part0, "dev/icon/16x16/plain/refresh.png", "Refresh Tree", function(e)
			{
				this.doTada();
				this.fillTree();
			}, this, this, "Refresh");
			dev.Statics.addToToolbar(part0, "dev/icon/16x16/plain/eye_blue.png", "View/Modify Tree Selection", this.viewTreeSelection, this, this, "View");
			dev.Statics.addToToolbar(part0, "dev/icon/16x16/plain/delete2.png", "Delete Tree Selection", this.deleteTreeSelection, this, this, "Delete");
		...
		getBrowserToolBar : function()
		{
			...
			dev.Statics.addToToolbar(part0, "dev/icon/16x16/plain/eye_blue.png", "View/Modify Tree Selection", this.viewBrowseTreeSelection, this, this, "View");
			this.customizeToolbar(toolbar);
		...
		getMainToolBar : function()
		{
			...
			var appsMenu = new qx.ui.toolbar.MenuButton("Applications", "dev/icon/16x16/plain/signpost.png", this.getAppsMenu());
			...
			var toolsMenu = new qx.ui.toolbar.MenuButton("Tools", "dev/icon/16x16/plain/carabiner.png", toolsmenu);
			toolsMenu.setToolTipText("Operate this application");

			// create connection menu button
			this.serverMenu = new qx.ui.toolbar.MenuButton("Server: Not Connected", "dev/icon/16x16/plain/server_delete.png", this.getServerMenu(this));
			...
			if (props["CurrentConnection"] !== undefined)
			{
				...
				this.serverMenu.setIcon("dev/icon/16x16/plain/server_connection.png")
				this.addCloseConnection(this);
			}
		...
		{
			var menu = new qx.ui.menu.Menu;
			this.addMenuButton(menu, "Control+Alt+R", "Refresh List", "dev/icon/16x16/plain/refresh.png", null, this.fillTree);
			return menu;
		},
