...
qx.Class.define("dev.test.SimpleTestUtil",
{
	...
	members :
	{
		...
		compareArraySizes : function(expected, actual) {
			this._test.assertEquals(expected.length, actual.length, "Should have " + expected.length + ", found: " + actual.length);
		},
		compareArrayContents : function(expected, actual)
		{
			...
			for (var i = 0, l = minSizeOfArrays; i < l; i++) {
				this._test.assertEquals(expected[i], actual[i], "item[" + i + "] should be " + expected[i]);
			}
		},
