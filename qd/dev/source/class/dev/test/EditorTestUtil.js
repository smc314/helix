...
qx.Class.define("dev.test.EditorTestUtil",
{
	...
	members :
	{
		...
		connect : function()
		{
			var connect_string = this._test.getServerHost() + "/" + this._test.getServerPort() + "/" + this._test.getServerUser() + "/" + this._test.getServerPass();
		...
		saveFails : function()
		{
			...
			repo.test = this;
			this.tryWaitForDialog(null, 10, 2500, repo, function(dialog)
			{
		...
		pressEditorToolbar : function(editor, buttonName)
		{
			...
			if (child1 instanceof qx.ui.toolbar.ToolBar)
			{
				...
				if (part1 instanceof qx.ui.toolbar.Part)
				{
					...
					{
						if (children[i] instanceof qx.ui.toolbar.Separator) {
							continue;
						}
		...
		checkTableValues : function(tableWidget, value, name) {
			for (var i = 0, l = value.length; i < l; i++)
			{
				...
				this.checkTableCellValue(tableWidget, cellCheck.row, cellCheck.col, cellCheck.value, name);
			}
		},
		...
		checkTableCellValue : function(tableWidget, row, col, value, name)
		{
			...
			this._test.assertEquals(value, tableCellVal, "Table (" + name + ") value at row (" + row + ") col (" + col + ") does not match.  Expected (" + value + ") actual (" + tableCellVal + ")");
		},
