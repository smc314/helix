	...
	members :
	{
		...
		*/
		...
		test0000_setServer : function()
		{
			...
			{
				dev.test.TestStatics.SetServerConnection(this.getServerHost(), this.getServerPort(), this.getServerUser(), this.getServerPass());
		...
		doubleClick : function(widg) {
			widg.fireEvent("dblclick", qx.event.type.Mouse, [ {

			}, widg, widg, false, true]);
		},
		...
		checkTableCellValue : function(tableWidget, row, col, value, name)
		{
			...
			this.assertEquals(value, tableCellVal, "Table (" + name + ") value at row (" + row + ") col (" + col + ") does not match.  Expected (" + value + ") actual (" + tableCellVal + ")");
		}
	},
	statics : {
