	...
	statics: {
		...
		*/
		getTextByValue: function (dropDown, value) {
			...
			var li = dev.Statics.findListItem(dropDown.getChildControl("list"), value);
			if (li === null) {
				return "";
			} else {
				return li.getLabel();
		...
		*/
		setDropDownByValue: function (dropDown, value) {
			...
			var list = dropDown.getChildControl("list");
			var li = dev.Statics.findListItem(list, value);
		...
		*/
		findQXParent: function (child, parentType) {
			...
			if (parent instanceof parentType) {
				...
				return dev.Statics.findQXParent(parent, parentType);
			}
		},
		...
		*/
		...
		/** Use this to set the dev data object into a tree node.  This handles
		...
		*/
		...
		verifyAllFields: function (theThis) {
			...
			for (var i = 0, l = theThis.fieldVerificationList.length; i < l; i++) {
				...
				var verifyResult = 0;
				if (obj.type === "string") {
					verifyResult = dev.Statics.verifyStringField(theThis, obj);
				} else if (obj.type === "int") {
					verifyResult = dev.Statics.verifyIntegerField(theThis, obj);
				} else {
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						...
						dev.Statics.loadOneField(theThis, dataObject, m);
					}
				}
			}
		...
		pullAllFields: function (theThis, dataObject) {
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						// This is a member that ends in "Field".  Load data for it:
						dev.Statics.pullOneField(theThis, dataObject, m);
					}
				}
			}
		...
		*/
		...
		getUserFromCurConnection : function() {
			var uprops = dev.Singleton.getInstance().getUserProperties();
