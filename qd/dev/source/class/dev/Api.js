	...
	statics :
	{
		...
		returnHasErrors : function(response, doAlert, callback, theThis)
		{
			...
			if (error_response !== null && error_response === "true")
			{
				if (doAlert)
				{
					...
					if (errors.length > 1) {
						dev.Statics.doAlert("Errors occurred during processing:<br/><br/>" + dev.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					} else {
						dev.Statics.doAlert("Error occurred during processing:<br/><br/>" + dev.Statics.formatSQLError(err_msg), "Alert", callback, theThis);
					}
				}
				return true;
			}
		...
		SendRequest : function(requestDoc, requestName, callback, theThis, errorCallback)
		{
			...
			req.onreadystatechange = qx.lang.Function.bind(function()
			{
				...
				if (dev.Api.CheckNullResponse(requestName, req, theThis) === true)
				{
					...
					{
						dev.Statics.doAlert("Error communicating with server.<br/><br/>Request: " + requestName + "<br/><br/>Failed to receive a response from the server.");
						if (theThis) {
		...
		ReAuthenticate : function(requestDoc, requestName, callback, theThis)
		{
			...
			tmpWindow.addOkEventListener("execute", function()
			{
				...
				}, null, function() {                        // the error callback
		...
		/** This function will call the /logic/utils/ConnectToServer
		  * server API.
		  */
		ConnectToServer : function(inputObj, completionFunction, theThis, errorFunction)
		{
			...
			dev.Api.SendRequest(requestDoc, "/logic/utils/ConnectToServer", completionFunction, theThis, errorFunction);
		},

		/** This function will call the ExecuteNamedQuery server API passing
		  * in a document that requires no input parameters.
		  */
		ExecuteNamedQuery0 : function(queryName, completionFunction, theThis)
		{
			//First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/ExecuteNamedQuery", completionFunction, theThis);
		},

		/** This function will call the ExecuteNamedQuery server API passing
		  * in a document that requires one input parameter.
		  */
		ExecuteNamedQuery1 : function(queryName, inputName, inputValue, completionFunction, theThis)
		{
			//First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "ExecuteNamedQuery");
			var requestRoot = requestDoc.documentElement;
		...
		/** This function will call the ExecuteNamedQuery server API passing
		  ...
		  */
		ExecuteNamedQuery2 : function(queryName, inputName, inputValue, input2Name, input2Value, completionFunction, theThis)
		{
			//First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "ExecuteNamedQuery");
			var requestRoot = requestDoc.documentElement;
		...
		/** This function will call the Authenticate server
		  ...
		  */
		Authenticate : function(user, pass, dbname, completionFunction, theThis, errorFunction)
		{
			//First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/Authenticate", completionFunction, theThis, errorFunction);
		},

		/** This function will call the UserProperties server API
		  */
		UserProperties : function(setList, delList, theThis, finalCallback)
		{
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/UserProperties", // User properties always use the same completion function.
		...
		/** This function will call the SystemProperties server API
		  */
		SystemProperties : function(theThis, finalCallback)
		{
			var requestDoc = qx.xml.Document.create(null, "SystemProperties");
			var requestRoot = requestDoc.documentElement;
			dev.Api.SendRequest(requestDoc, "/SystemProperties", // System properties always use the same completion function.
		...
		/** This function will call the SaveLayout server API
		  */
		SaveLayout : function(layoutDoc, layoutName, completionFunction, theThis)
		{
			var requestRoot = layoutDoc.documentElement;
			requestRoot.setAttribute("filename", layoutName);

			// Now send the request to the server.
			dev.Api.SendRequest(layoutDoc, "/logic/utils/SaveLayout", completionFunction, theThis);
		},
		...
		/** This function will call the /logic/dev/GetProject
		  * server API.
		  */
		GetProject : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/dev/GetProject", completionFunction, theThis);
		},

		/** This function will call the /logic/dev/SaveProject
		  * server API.
		  */
		SaveProject : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/dev/SaveProject", completionFunction, theThis);
		},

		/** This function will call the /logic/dev/GetProjectList
		  * server API.
		  */
		GetProjectList : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetProjectList");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/dev/GetProjectList", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetLogMsgCount
		  * server API.
		  */
		GetLogMsgCount : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetLogMsgCount", completionFunction, theThis);
		},

		/** This function will call the /logic/util/CloseConnection
		  * server API.
		  */
		CloseConnection : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "CloseConnection");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/CloseConnection", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetAllLayouts
		  * server API.
		  */
		GetAllLayouts : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetAllLayouts");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetAllLayouts", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetLogSettings
		  * server API.
		  */
		GetLogSettings : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetLogSettings");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetLogSettings", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetLogMessagesByRange
		  * server API.
		  */
		GetLogMessagesByRange : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetLogMessagesByRange", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetHitMap
		  * server API.
		  */
		GetHitMap : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetHitMap");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetHitMap", completionFunction, theThis);
		},

		/** This function will call the /logic/util/CleanoutLRTask
		  * server API.
		  */
		CleanoutLRTask : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/CleanoutLRTask", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetAllLRTasks
		  * server API.
		  */
		GetAllLRTasks : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetAllLRTasks");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetAllLRTasks", completionFunction, theThis);
		},

		/** This function will call the /logic/util/SaveLogMsg
		  * server API.
		  */
		SaveLogMsg : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/SaveLogMsg", completionFunction, theThis);
		},

		/** This function will call the /logic/util/FindLogMessage
		  * server API.
		  */
		FindLogMessage : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/FindLogMessage", completionFunction, theThis);
		},

		/** This function will call the /logic/util/UpdateLogSettings
		  * server API.
		  */
		UpdateLogSettings : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/UpdateLogSettings", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetProfile
		  * server API.
		  */
		GetProfile : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetProfile");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetProfile", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetOneLRTask
		  * server API.
		  */
		GetOneLRTask : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetOneLRTask", completionFunction, theThis);
		},

		/** This function will call the /logic/util/SwitchServerConnection
		  * server API.
		  */
		SwitchServerConnection : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/SwitchServerConnection", completionFunction, theThis);
		},

		/** This function will call the /logic/util/ExecuteGenericSQL
		  * server API.
		  */
		ExecuteGenericSQL : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/ExecuteGenericSQL", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetLogMessages
		  * server API.
		  */
		GetLogMessages : function(inputObj, completionFunction, theThis)
		{
			// First build the request XML Document
			...
			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetLogMessages", completionFunction, theThis);
		},

		/** This function will call the /logic/util/GetLogFiles
		  * server API.
		  */
		GetLogFiles : function(completionFunction, theThis)
		{
			// First build the request XML Document
			var requestDoc = qx.xml.Document.create(null, "GetLogFiles");

			// Now send the request to the server.
			dev.Api.SendRequest(requestDoc, "/logic/util/GetLogFiles", completionFunction, theThis);
		},
