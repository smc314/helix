...
/* ************************************************************************
#asset(dev/icon/64x64/shadow/server_to_client.png)
************************************************************************ */
...
/** This class defines a dialog that presents a list box
  ...
  */
qx.Class.define("dev.dialog.LRTaskDialog",
{
	...
	{
		this.lrTask = lrTask;
		this.icon = "dev/icon/64x64/shadow/server_to_client.png";
	...
	members :
	{
		...
		doFormLayout : function(layout)
		{
			...
			layout.add(a1);
			this.pb = new qx.ui.indicator.ProgressBar(this.lrTask.getTaskStart(), this.lrTask.getTaskFinish());
		...
		runLongPoll : function(userData, timerId)
		{
			...
			req.onreadystatechange = qx.lang.Function.bind(function()
			{
				...
				if (dev.Api.returnHasErrors(response, true, this.errorCallback, this.callbackThis))
				{
				...
				if (response.nodeName === "GetOneLRTask")
				{
					...
					var objectList = dev.sqldo.LRTask.readElementChildren(response);
