...
qx.Class.define("dev.proj.ProjectEditor",
{
	...
	construct : function(object_id)
	{
		...
		{
			name : "ProjectEditor.MainPage.xml",
			doc : null
		});
		this.dynamicLayouts.push(
		{
			name : "ProjectEditor.NextPage.xml",
			doc : null
		});
	...
	members :
	{
		...
		initializeToolbar : function(tb)
		{
			...
			dev.Statics.addToToolbarWithShortcut(part, "dev/icon/16x16/plain/refresh.png", "Reload the project from the filesystem", this.doReload, this, this, null, "Reload");
			tb.add(part);
		...
		loadDataIntoForm : function()
		{
			...
			// Bind each of our layout pages to the data object that

			// we have created or received from the server.
			dev.DataBinding.bindLayout(this, this.dataObject, this.getDynamicLayout("ProjectEditor.MainPage.xml"));
			dev.DataBinding.bindLayout(this, this.dataObject, this.getDynamicLayout("ProjectEditor.NextPage.xml"));
			...
			/* ******************************************************* */
			this.overview_label1.setLabel("Overview Label1: " + this.dataObject.getProjName());
