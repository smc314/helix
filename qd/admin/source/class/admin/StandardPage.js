...
#asset(admin/icon/16x16/plain/window_delete.png)
************************************************************************ */
qx.Class.define("admin.StandardPage", {
	...
	construct : function () {
		...
		this.icon = "admin/icon/32x32/shadow/sign_warning.png";
	...
	properties : {
		 initialiadmin : {init : false, check : "Boolean" }
	},
	...
	members : {
		...
		setupToolbarAndTabs : function( widget ) {
			...
			tabview.addListener("drop", admin.StandardPage.handleDragDrop);
			tabview.addListener("dragover", admin.StandardPage.handleDragOver);
		...
		rmcCloseTab : function ( e ){
			...
			var tabview = admin.Statics.findQXParent( btn, qx.ui.tabview.TabView );
			var pageSearched = admin.StandardPage.findTabPage( btn );
			if(pageSearched){
		...
		initializeToolbar : function( ) {
			...
			var split_vertical = new qx.ui.toolbar.Button(null, "admin/icon/16x16/plain/window_split_ver.png");
			split_vertical.setUserData( "admin.CurrentPage", this );
			...
			var split_horizontal = new qx.ui.toolbar.Button(null, "admin/icon/16x16/plain/window_split_hor.png");
			split_horizontal.setUserData( "admin.CurrentPage", this );
			...
			var split_close = new qx.ui.toolbar.Button(null, "admin/icon/16x16/plain/window_delete.png");
			split_close.setUserData( "admin.CurrentPage", this );
		...
		createComandCreateView : function ( callback ){
			...
			c1.addListener("execute", function(event) {
				...
				var parentToolbar = admin.Statics.findQXParent( menu_Button, qx.ui.toolbar.ToolBar );
				...
				var tab_page = admin.StandardPage.addStandardTab( tabview, event.getData().getLabel(), true);
		...
		initialTabView : function( tabView ) {
			...
			var p1_1 = admin.StandardPage.addStandardTab( tabView, "View 1", true);
			var p1_2 = admin.StandardPage.addStandardTab( tabView, "View 2", false);
			var p1_3 = admin.StandardPage.addStandardTab( tabView, "View 3", false);
		...
		doSplitVertical : function( ) {
			...
			//        -> toolbar part
			//             -> toolbar button === this.
			var parentToolbar = admin.Statics.findQXParent( this, qx.ui.toolbar.ToolBar );
			var layoutParent = parentToolbar.getLayoutParent();
			var current_page = this.getUserData("admin.CurrentPage");
		...
		doCloseSplit : function( ) {
			...
			var our_toolbar = admin.Statics.findQXParent( this, qx.ui.toolbar.ToolBar );
			var our_area = admin.Statics.findQXParent(our_toolbar, qx.ui.container.Composite);
			var our_splitpane = admin.Statics.findQXParent(our_area, qx.ui.splitpane.Pane);
			var other_area = null;
			if( admin.Statics.getSplitFirst(our_splitpane) === our_area ){
				// we are the first area.
				other_area = admin.Statics.getSplitSecond(our_splitpane);
			} else {
				// we are the second area.
				other_area = admin.Statics.getSplitFirst(our_splitpane);
			}
			...
			var ourPage = null;
			if(splitpane_parent instanceof admin.StandardPage){
				...
				ourPage = admin.Statics.findQXParent( splitpane_parent, admin.StandardPage );
			}
	...
	statics : {
		...
		findTabPage : function (tab_button) {
			var tabview = admin.Statics.findQXParent( tab_button, qx.ui.tabview.TabView );
			if(!tabview){
		 ...
		 * closetab event (pointing it to admin.StandardPage.closeTabHandler) and it will
		...
		addStandardTab : function ( tabview, tabName, set_as_checked) {
			...
			var our_spage = admin.Statics.findQXParent( tabview, admin.StandardPage );
			...
			tab_page.getButton().addListener("close", admin.StandardPage.closeTabHandler, true );


			tab_page.getButton().setDraggable(true);
			tab_page.getButton().addListener("dragstart", admin.StandardPage.handleDragStart);
			tab_page.getButton().addListener("droprequest", admin.StandardPage.handleDropRequest);

			tab_page.setDroppable(true);
			tab_page.addListener("drop", admin.StandardPage.handleDragDrop);
			tab_page.addListener("dragover", admin.StandardPage.handleDragOver);
		...
		handleDragDrop : function ( e ) {
			...
			var orig_tabview = admin.Statics.findQXParent( orig_button, qx.ui.tabview.TabView );
			var orig_page = admin.StandardPage.findTabPage( orig_button );
			...
			if(dropping_where instanceof qx.ui.tabview.TabView){
				...
				new_tabview = admin.Statics.findQXParent( dropping_where, qx.ui.tabview.TabView );
			}
