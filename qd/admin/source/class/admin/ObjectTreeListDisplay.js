...
************************************************************************ */
/* ************************************************************************
#asset(admin/icon/basic_foundation/16x16/plain/refresh.png)
#asset(admin/icon/basic_foundation/32x32/shadow/sign_warning.png)
************************************************************************ */
qx.Class.define("admin.ObjectTreeListDisplay", {
	...
	members : {
		...
		initializeToolbar : function( tb ) {
			...
			// Refresh button
			admin.Statics.addToToolbar( this.toolbar_part1, "admin/icon/basic_foundation/16x16/plain/refresh.png",
		...
		createRMCMenu : function(ev) {
			...
			// refresh
			admin.Statics.addToRMCMenu( this.rmc, "Refresh",
				'admin/icon/basic_foundation/16x16/plain/refresh.png', this.rmcFullRefresh, this);

			// edit
			admin.Statics.addToRMCMenu( this.rmc, "Edit...",
				'admin/icon/basic_foundation/16x16/plain/edit.png', this.rmcEdit, this);

			// Add New...
			if(this.doAddNew){
				admin.Statics.addToRMCMenu( this.rmc, "Add New...",
					'admin/icon/basic_foundation/16x16/plain/document_new.png', this.addNew, this);
			}

			// Delete...
			if(this.deleteApi !== null){
				admin.Statics.addToRMCMenu( this.rmc, "Delete...",
					'admin/icon/basic_foundation/16x16/plain/delete2.png', this.doDelete, this);
			}
		...
		rmcEdit : function(ev) {
			...
			var page = admin.StandardPage.addStandardTab( targetTabView ,
		...
		getTargetTabView : function () {
			...
			var parentTaskPage = admin.Statics.findQXParent( this, admin.StandardPage );
			if(parentTaskPage === null){
				targetTabView = admin.Statics.findQXParent( this, qx.ui.tabview.TabView );
			} else {
		...
		/** This will set the data object on the tree node that has the given ID.
		  ...
		  */
		setadminDataObject : function ( nodeID, dataObject ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				node.adminDataObject = dataObject;
			}
		},
		...
		/** This will get the data object on the tree node that has the given ID.
		  * The ID comes from calls to dataModel.addBranch().
		  */
		getadminDataObject : function ( nodeID ){
			var node = this.tree.getTableModel().getData()[ nodeID ];
			if(node){
				return node.adminDataObject;
			} else {
				return null;
			}
		},
		...
		addNew : function(ev) {
			...
			var page = admin.StandardPage.addStandardTab( this.getTargetTabView(),
		...
		doDelete : function(ev) {
			...
			var windowIcon = "admin/icon/basic_foundation/32x32/shadow/sign_warning.png";
			var windowLabel = "Are you sure you want to delete the selected items?";
			var tmpWindow = new admin.dialog.QuestionDialog( windowTitle, windowIcon,
				windowLabel );
	...
	destruct : function () {
		...
		admin.Statics.cleanupStandardTable( this.tree );
