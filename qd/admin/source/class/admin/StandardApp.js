...
************************************************************************ */
/* ************************************************************************
#asset(admin/icon/16x16/plain/home.png)
#asset(admin/icon/16x16/plain/control_panel.png)
#asset(admin/icon/16x16/plain/compass.png)
#asset(admin/icon/16x16/plain/house.png)
#asset(admin/icon/16x16/plain/help2.png)
#asset(admin/icon/16x16/plain/question_and_answer.png)
#asset(admin/icon/16x16/plain/book_blue_open2.png)
...
#asset(admin/icon/16x16/plain/house.png)
#asset(admin/icon/16x16/plain/download.png)
#asset(admin/helix-about-background.png)
#asset(admin/icon/16x16/plain/folder.png)
...
#asset(admin/icon/16x16/plain/refresh.png)
#asset(admin/icon/16x16/plain/eye_blue.png)
#asset(admin/icon/16x16/plain/signpost.png)
#asset(admin/icon/16x16/plain/carabiner.png)
#asset(admin/icon/16x16/plain/server_delete.png)
#asset(admin/icon/16x16/plain/server_connection.png)
#asset(admin/icon/16x16/plain/refresh.png)
************************************************************************ */
qx.Mixin.define("admin.StandardApp", {
	...
	members: {
		...
		*/
		setupStandardApp: function (skipSystem, customSplitPane) {
			...
			var self = this;
			admin.PageVisibility.getInstance().on("change",
			function() {
				if (admin.PageVisibility.getInstance().isHidden()) {
					// do something when hidden
				} else {
					// do something when the page gets back to visible
					admin.Api.UserProperties({}, {}, self,
					function () {
						...
						if (label != props["CurrentConnection"] ) {
							...
							admin.Api.SwitchServerConnection(obj,
							function (response) {
		...
		loadSystemProperties: function () {
			if (this.skipSystem !== undefined && this.skipSystem === true) {
				...
				admin.Api.SystemProperties(this, function () {
		...
		*/
		getAppsMenu: function () {
			...
			var welcome = new qx.ui.menu.Button("Welcome To Helix Development",
					"admin/icon/16x16/plain/home.png");
			admin.Statics.setHtmlID( welcome, "Welcome");
			var adminBtn = new qx.ui.menu.Button("Helix Administration",
					"admin/icon/16x16/plain/control_panel.png");
			admin.Statics.setHtmlID( adminBtn, "Admin");
			var devBtn = new qx.ui.menu.Button("Helix Development",
					"admin/icon/16x16/plain/compass.png");
			admin.Statics.setHtmlID( devBtn, "Dev");
		...
		getHelpMenu: function () {
			...
			aboutSPButton.addListener("execute", function () {
				...
				if (tab_page != null) { // brand new
					tab_page.setIcon("admin/icon/16x16/plain/about.png");
		...
		*/
		getServerMenu: function () {
			...
			var btnNew = new qx.ui.menu.Button("New Connection...",
					"admin/icon/16x16/plain/server_connection.png");
			admin.Statics.setHtmlID( btnNew, "NewConnection");
			...
			var btnMng = new qx.ui.menu.Button("Manage Connections",
					"admin/icon/16x16/plain/server_edit.png");
			admin.Statics.setHtmlID( btnMng, "ManageConnections");
				...
				if (props.hasOwnProperty(p)) {
					...
					if (idx1 != -1 && idx2 != -1 && idx2 > idx1) {
						...
						btn.addListener("execute", function (e) {
							...
							admin.Api.SwitchServerConnection(obj, function (response) {
								this.serverMenu.setLabel(label);
								this.addCloseConnection();
								this.loadSystemProperties();
								this.closeAllEditors();
		...
		displayServerConnectDialog: function () {
			...
			tmpWindow.addOkEventListener("execute", function () {
				...
				admin.Api.ConnectToServer(obj, function (response) {
		...
		manageServerConnectDialog : function(){
			...
			var tmpWindow = new admin.dialog.ManageServerConnections();
			tmpWindow.open();
			qx.core.Init.getApplication().getRoot().add(tmpWindow);
		...
		*/
		addToServerMenu: function (newLabel) {
			...
			var btn = new qx.ui.menu.Button(newLabel, "admin/icon/16x16/plain/server_connection.png");
			admin.Statics.setHtmlID( btn, newLabel);
		...
		addCloseConnection: function () {
			...
			var closeBtn = new qx.ui.menu.Button("Close Current Connection",
				"admin/icon/16x16/plain/server_delete.png");
			admin.Statics.setHtmlID( closeBtn, "CloseCurrentConnection");
			closeBtn.addListener("execute", function () {
				admin.Api.CloseConnection(function (response) {
					this.serverMenu.setLabel("Server: Not Connected");
					this.serverMenu.setIcon("admin/icon/16x16/plain/server_delete.png");
					this.pruneTree();
				}, this);
			}, this);
		...
		addEditingTab: function (tabName) {
			...
			for (var i = 0, l = pages.length; i < l; i++) {
				if (pages[i].getLabel() === tabName ||
					...
					admin.Statics.doAlertGreen("A tab is already open with the name: " + tabName);
					return null; // it's already there
				}
			}

			// Didn't find it, so go ahead and add a new tab for it:
			var page = admin.Statics.addTabPage(this.tabView, tabName, true, true);
			return page;
		},
		...
		*/
		...
		showHelp: function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("admin/icon/16x16/plain/help2.png");
				var iframe = new qx.ui.embed.Iframe().set({
		...
		showDocs: function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("admin/icon/16x16/plain/book_blue_open2.png");
				var iframe = new qx.ui.embed.Iframe().set({
					source: "/devdoc/html/index.html"
				});
				tab_page.add(iframe, { flex: 10 });
		...
		showQA: function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("admin/icon/16x16/plain/question_and_answer.png");
				var iframe = new qx.ui.embed.Iframe().set({
		...
		showWelcome: function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("admin/icon/16x16/plain/house.png");
				tab_page.add(new admin.HomePage(0), { flex: 10 });
			}
		},
		...
		showUpdates : function () {
			...
			if (tab_page != null) { // brand new
				tab_page.setIcon("admin/icon/16x16/plain/download.png");
				tab_page.add(new admin.help.CheckForUpdates(0), { flex: 10 });
			}
		},
		...
		*/
		commonAbout: function () {
			...
			decorator.setBackgroundImage( "admin/helix-about-background.png" );
			var tmpWindow = new admin.dialog.MessageDialog(windowTitle, windowIcon, windowText);
		...
		getSplitPane: function () {
			...
			admin.Statics.setHtmlID( this.tree, "ExplorerTree" );
			...
			var navTab = admin.Statics.addEditorSubTab( this.treeTabs, "Explorer", true, this.getTreeToolBar() );
			admin.Statics.findQXParent( navTab, qx.ui.tabview.Page ).setIcon("admin/icon/16x16/plain/folder.png" );
		...
		createTreeRMC: function () {
			...
			admin.Statics.addToRMCMenu(this.treeRMC, "Refresh Tree",
				"admin/icon/16x16/plain/refresh.png", this.fillTree, this);
			admin.Statics.addToRMCMenu(this.treeRMC, "View Selected Item",
				"admin/icon/16x16/plain/eye_blue.png", this.viewTreeSelection, this);
			admin.Statics.addToRMCMenu(this.treeRMC, "Delete Selected Item",
		...
		viewTreeSelection: function () {
			if (this.tree.getSelection().length === 0) {
				admin.Statics.doAlert("There is nothing selected in the tree.",
			...
			if (treeNode.editObject){
				...
				admin.Statics.doAlert("There is nothing to view for the current selection.",
					"Alert", this.setTreeFocus, this);
			}
		...
		getTreeToolBar: function () {
			...
			admin.Statics.addToToolbar(part0, "admin/icon/16x16/plain/refresh.png",
				"Refresh Tree", function(e) {this.doTada(); this.fillTree();}, this, this, "Refresh");

			admin.Statics.addToToolbar(part0, "admin/icon/16x16/plain/eye_blue.png",
				"View/Modify Tree Selection", this.viewTreeSelection, this, this, "View");

			admin.Statics.addToToolbar(part0, "admin/icon/16x16/plain/delete2.png",
		...
		getMainToolBar: function () {
			...
			var appsMenu = new qx.ui.toolbar.MenuButton("Applications", "admin/icon/16x16/plain/signpost.png", this.getAppsMenu());
			appsMenu.setToolTipText("Switch between the Helix applications");
			admin.Statics.setHtmlID( appsMenu, "Applications" );
			...
			var toolsMenu = new qx.ui.toolbar.MenuButton("Tools", "admin/icon/16x16/plain/carabiner.png", toolsmenu);
			toolsMenu.setToolTipText("Operate this application");
			admin.Statics.setHtmlID( toolsMenu, "Tools" );

			// create connection menu button
			this.serverMenu = new qx.ui.toolbar.MenuButton("Server: Not Connected", "admin/icon/16x16/plain/server_delete.png", this.getServerMenu(this));
			this.serverMenu.setToolTipText("Control the Helix database connection");
			admin.Statics.setHtmlID( this.serverMenu, "ServerMenu" );
			...
			if (props["CurrentConnection"] !== undefined) {
				...
				this.serverMenu.setIcon("admin/icon/16x16/plain/server_connection.png")
				this.addCloseConnection(this);
			}
		...
		getFileMenu: function () {
			...
			this.addMenuButton(menu, "Control+Alt+R", "Refresh List",
				"admin/icon/16x16/plain/refresh.png", null, this.fillTree);
