...
#asset(admin/icon/64x64/shadow/clients.png)
#asset(admin/icon/16x16/plain/barcode.png)
************************************************************************ */
qx.Class.define("admin.admin.ServerSessions", {
	extend: admin.ObjectEdit,

	construct : function ( object_id ) {
		this.dynamicLayouts = [];
		this.dynamicLayouts.push({name: "ServerSessions.MainPage.xml", doc:null});
		this.dynamicLayouts.push({name: "ServerSessions.NextPage.xml", doc:null});

		if(object_id === 0){
			this.dataObject = new admin.sqldo.Table();
			this.dataObject.setTName("Ivory Director Server Sessions");
		} else {
	...
	members : {
		...
		createMainTab: function(tab_page)
		{
			...
			root.add(this.initGeneralStatus(
				"Server Sessions",  // This is the large title in the overview section
				"admin/icon/64x64/shadow/clients.png", // Large icon in overview
			...
			admin.layout.LayoutEngine.renderLayout( this,
				this.getDynamicLayout("ServerSessions.MainPage.xml"),
				live );
		...
		{
			admin.layout.LayoutEngine.renderLayout( this,
				this.getDynamicLayout("ServerSessions.NextPage.xml"),
				tab_page );
