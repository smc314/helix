...
#asset(admin/icon/64x64/shadow/server_edit.png)
#asset(admin/icon/16x16/plain/barcode.png)
************************************************************************ */
qx.Class.define("admin.admin.ServerSettings", {
	extend: admin.ObjectEdit,

	construct : function ( object_id ) {
		this.dynamicLayouts = [];
		this.dynamicLayouts.push({name: "ServerSettings.MainPage.xml", doc:null});
		this.dynamicLayouts.push({name: "ServerSettings.NextPage.xml", doc:null});

		if(object_id === 0){
			this.dataObject = new admin.sqldo.Table();
			this.dataObject.setTName("Ivory Director Server Settings");
		} else {
	...
	members : {
		...
		createMainTab: function(tab_page)
		{
			...
			root.add(this.initGeneralStatus(
				"Server Settings",  // This is the large title in the overview section
				"admin/icon/64x64/shadow/server_edit.png", // Large icon in overview
			...
			admin.layout.LayoutEngine.renderLayout( this,
				this.getDynamicLayout("ServerSettings.MainPage.xml"),
				live );
		...
		{
			admin.layout.LayoutEngine.renderLayout( this,
				this.getDynamicLayout("ServerSettings.NextPage.xml"),
				tab_page );
