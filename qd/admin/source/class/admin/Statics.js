...
#asset(admin/icon/32x32/shadow/ok.png)
************************************************************************ */
qx.Class.define("admin.Statics", {
	...
	statics: {
		...
		*/
		populateDropDown: function (dropDown, xmlDoc, elementName,
			...
			if (autoSelectItem !== undefined) {
				admin.Statics.selectDropDownIndex(dropDown, autoSelectItem);
			}
		...
		*/
		getTextByValue: function (dropDown, value) {
			...
			var li = admin.Statics.findListItem(dropDown.getChildControl("list"), value);
			if (li === null) {
				return "";
			} else {
				return li.getLabel();
		...
		*/
		setDropDownByValue: function (dropDown, value) {
			...
			var list = dropDown.getChildControl("list");
			var li = admin.Statics.findListItem(list, value);
		...
		*/
		setDropDownByLabel: function (dropDown, label) {
			...
			var list = dropDown.getChildControl("list");
			var li = admin.Statics.findListItemByLabel(list, label);
		...
		*/
		...
		getDDSelectedValue: function (dropDown) {
			if (admin.Statics.isDropDownUnSelected(dropDown)) {
				return null;
			}
		...
		* <pre>
		var parent = admin.Statics.findQXParent( child, qx.ui.table.Table );
		* </pre>
		*/
		findQXParent: function (child, parentType) {
			...
			if (parent instanceof parentType) {
				...
				return admin.Statics.findQXParent(parent, parentType);
			}
		},
		...
		*/
		...
		xmlGetBase64 : function(elem) {
			var b64 = admin.Statics.xmlGetCDATASection( elem );
			return admin.Statics.decode64( b64 );
		},
		...
		xmlSetBase64 : function(parent, contents){
			var b64 = admin.Statics.encode64( contents );
			admin.Statics.xmlSetCDATASection( parent, b64 );
		},
		...
		*/
		addFieldsToForm: function (theThis, layout,
			...
			var l1 = new qx.ui.basic.Label(label1Name); admin.Statics.trackExtraObj(theThis, l1);
			...
			admin.Statics.trackNamedAttr(theThis, field1Name);
			admin.Statics.setHtmlID( f1, field1Name );

			if (label2Name) {
				var l2 = new qx.ui.basic.Label(""); admin.Statics.trackExtraObj(theThis, l2);
				l2.setWidth(5);
				hb.add(l2);

				var l3 = new qx.ui.basic.Label(label2Name); admin.Statics.trackExtraObj(theThis, l3);
				...
				if (field2Name) {
					...
					admin.Statics.trackNamedAttr(theThis, field2Name);
					admin.Statics.setHtmlID( f2, field2Name );
				}
			}

			if (label3Name) {
				var l4 = new qx.ui.basic.Label(""); admin.Statics.trackExtraObj(theThis, l4);
				l4.setWidth(5);
				hb.add(l4);

				var l5 = new qx.ui.basic.Label(label3Name); admin.Statics.trackExtraObj(theThis, l5);
				...
				if (field3Name) {
					...
					admin.Statics.trackNamedAttr(theThis, field3Name);
					admin.Statics.setHtmlID( f3, field3Name );
				}
			}

			if (label4Name) {
				var l6 = new qx.ui.basic.Label(""); admin.Statics.trackExtraObj(theThis, l6);
				l6.setWidth(5);
				hb.add(l6);

				var l7 = new qx.ui.basic.Label(label4Name); admin.Statics.trackExtraObj(theThis, l7);
				...
				if (field4Name) {
					...
					admin.Statics.trackNamedAttr(theThis, field4Name);
					admin.Statics.setHtmlID( f4, field4Name );
				}
			}

			var l8 = new qx.ui.basic.Label(""); admin.Statics.trackExtraObj(theThis, l8);
			l8.setWidth(2);
			hb.add(l8);
		...
		*/
		addTextFieldsToForm: function (theThis, layout,
			...
			admin.Statics.trackExtraObj(theThis, l1);
			l1.setWidth(label1Size);
			hb.add(l1);
			...
			if (label3Name) {
				...
				var l3 = new qx.ui.basic.Label(label3Name); admin.Statics.trackExtraObj(theThis, l3);
				...
				if (field3Name) {
					...
					admin.Statics.trackNamedAttr(theThis, field3Name);
					admin.Statics.setHtmlID( f2, field3Name );
				}
			}
			...
			if (label4Name) {
				...
				var l3 = new qx.ui.basic.Label(label4Name); admin.Statics.trackExtraObj(theThis, l3);
				...
				if (field4Name) {
					...
					admin.Statics.trackNamedAttr(theThis, field4Name);
					admin.Statics.setHtmlID( f2, field4Name );
				}
			}

			var l3 = new qx.ui.basic.Label("");
			admin.Statics.trackExtraObj(theThis, l3);
			l3.setWidth(2);
			hb.add(l3);
		...
		*/
		addTextArea: function (theThis, layout, flexHeight, labelText, textName) {
			...
			theThis[textName] = ta;
			admin.Statics.trackNamedAttr(theThis, textName);
			admin.Statics.setHtmlID( ta, textName );
		},
		...
		*/
		addLongLabel: function (layout, labelText, textName, theThis) {
			...
			if (textName) {
				theThis[textName] = l1;
				admin.Statics.setHtmlID( l1, textName );
			}
		},
		...
		*/
		addBoxWithButtons: function (theThis, layout, boxName, flexHeight, flexWidth,
			...
			admin.Statics.trackNamedAttr(theThis, boxName);
			admin.Statics.setHtmlID( list1, boxName);
			hb.add(list1, {flex:1});
			...
			if (button1Name) {
				...
				admin.Statics.trackNamedAttr(theThis, button1Name);
				admin.Statics.setHtmlID( b, button1Name);
				vb.add(b);
			}
			if (button2Name) {
				...
				admin.Statics.trackNamedAttr(theThis, button2Name);
				admin.Statics.setHtmlID( b, button2Name);
				vb.add(b);
			}
			if (button3Name) {
				...
				admin.Statics.trackNamedAttr(theThis, button3Name);
				admin.Statics.setHtmlID( b, button3Name);
				vb.add(b);
			}
			if (button4Name) {
				...
				admin.Statics.trackNamedAttr(theThis, button4Name);
				admin.Statics.setHtmlID( b, button4Name);
				vb.add(b);
			}
		...
		*/
		addCheckboxes: function (theThis, layout, leftSize,
			...
			var tmpLabel;
			if (initialLabel) {
				tmpLabel = new qx.ui.basic.Label(initialLabel); admin.Statics.trackExtraObj(theThis, tmpLabel);
			} else {
			...
			if (checkbox1Name) {
				...
				admin.Statics.trackNamedAttr(theThis, checkbox1Name);
				admin.Statics.setHtmlID( cb1, checkbox1Name);
			}
			if (checkbox2Name) {
				...
				admin.Statics.trackNamedAttr(theThis, checkbox2Name);
				admin.Statics.setHtmlID( cb2, checkbox2Name);
			}
			if (checkbox3Name) {
				...
				admin.Statics.trackNamedAttr(theThis, checkbox3Name);
				admin.Statics.setHtmlID( cb3, checkbox3Name);
			}
			if (checkbox4Name) {
				...
				admin.Statics.trackNamedAttr(theThis, checkbox4Name);
				admin.Statics.setHtmlID( cb4, checkbox4Name);
			}
		...
		*/
		addRadioGroupVertical: function (theThis, layout, leftSize,
			...
			if (r1Name) rg.add(admin.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r1Name, r1Label));
			if (r2Name) rg.add(admin.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r2Name, r2Label));
			if (r3Name) rg.add(admin.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r3Name, r3Label));
			if (r4Name) rg.add(admin.Statics.addRadioButtonVertical(theThis, layout,
				leftSize, r4Name, r4Label));
			if (r5Name) rg.add(admin.Statics.addRadioButtonVertical(theThis, layout,
		...
		addRadioButtonVertical: function (theThis, layout, leftSize, r1Name, r1Label
			...
			if (r1Name) {
				...
				hb.add(rb);
				theThis[r1Name] = rb;
				admin.Statics.trackNamedAttr(theThis, r1Name);
				admin.Statics.setHtmlID( rb, r1Name);
				layout.add(hb);
				return rb;
			} else {
				return null;
		...
		*/
		addRadioGroupHorizontal: function (theThis, layout, leftSize, groupName,
			...
			if (r2Name) {
				...
				hb.add(rb);
				theThis[r2Name] = rb;
				admin.Statics.trackNamedAttr(theThis, r2Name);
				admin.Statics.setHtmlID( rb, r2Name);
				rg.add(rb);
			}
			if (r3Name) {
				...
				hb.add(rb);
				theThis[r3Name] = rb;
				admin.Statics.trackNamedAttr(theThis, r3Name);
				admin.Statics.setHtmlID( rb, r3Name);
				rg.add(rb);
			}
			if (r4Name) {
				...
				hb.add(rb);
				theThis[r4Name] = rb;
				admin.Statics.trackNamedAttr(theThis, r4Name);
				admin.Statics.setHtmlID( rb, r4Name);
				rg.add(rb);
			}
			if (r5Name) {
				...
				hb.add(rb);
				theThis[r5Name] = rb;
				admin.Statics.trackNamedAttr(theThis, r5Name);
				admin.Statics.setHtmlID( rb, r5Name);
				rg.add(rb);
			}
			layout.add(hb);
			return rg;
		...
		*/
		createDoubleList: function (theThis, layout,
			...
			var l1 = new qx.ui.basic.Label(leftListLabel); admin.Statics.trackExtraObj(theThis, l1);
			hb1.add(l1);
			hb1.add(new qx.ui.core.Spacer, { flex: 1 });
			var l2 = new qx.ui.core.Spacer;
			l2.setWidth(middleSize + 3);
			hb1.add(l2);
			var l3 = new qx.ui.basic.Label(rightListLabel); admin.Statics.trackExtraObj(theThis, l3);
			hb1.add(l3);
			hb1.add(new qx.ui.core.Spacer, { flex: 1 });
			form_layout.add(hb1);
			...
			admin.Statics.trackNamedAttr(theThis, leftListName);
			admin.Statics.setHtmlID( list1, leftListName);
			...
			vb1.add(add_button);
			admin.Statics.setHtmlID( add_button, "Add");
			admin.Statics.setHtmlID( remove_button, "Remove");
			...
			admin.Statics.trackNamedAttr(theThis, rightListName);
			admin.Statics.setHtmlID( list2, rightListName);
			...
			add_button.addListener("execute", function () {
				...
				for (var i = 0, l = selected.length; i < l; i++) {
					...
					var match = admin.Statics.findListItem(list1, item.getModel());
					if (match === null) {
		...
		*/
		addToToolbar: function (toolbar, icon, tooltip, action, action_this, ownerObj, label) {
			...
			admin.Statics.setHtmlID( btn, label);
			//btn.setHeight("auto");
		...
		*/
		...
		/** Use this to destroy an array of admin data objects
		*
		*/
		...
		*/
		addTabPage: function (tabview, name, showClose, isChecked) {
			...
			admin.Statics.setHtmlID( tab_page.getButton(), name);
			return tab_page;
		},
		...
		*/
		...
		doAlert: function (message, title, doAfter, theThis) {
			...
			var tmpWindow = new admin.dialog.MessageDialog(windowTitle, windowIcon,
				message);
			if (doAfter) {
		...
		doAlertGreen: function (message, title, doAfter, theThis) {
			var windowTitle = title ? title : "Alert";
			var windowIcon = "admin/icon/32x32/shadow/ok.png";
			var tmpWindow = new admin.dialog.MessageDialog(windowTitle, windowIcon,
				message);
			if (doAfter) {
				tmpWindow.addOkEventListener("execute", doAfter, theThis);
		...
		/** Use this to set the admin data object into a tree node.  This handles
		* the setting in a standard way that makes it easy to set/get the admin
		...
		*/
		setTreeNodeDO: function (tree, nodeID, dataObject) {
			...
			realNode.adminDataObject = dataObject;
		},

		/** Use this to get the admin data object from a tree node.  Use the
		* setTreeNodeDO in order to use this method properly.
		*/
		getTreeNodeDO: function (tree, nodeID) {
			var realNode = tree.getTableModel().getData()[nodeID];
			return realNode.adminDataObject;
		},

		/** Use this to get the admin data object from a tree by using the row number.
		...
		*/
		...
		/** Use this to set the admin data object into a table row.  This will work
		* with the Simple table model.
		*/
		setTableRowDO: function (table, row, dataObject) {
			table.getTableModel().getRowData(row).adminDataObject = dataObject;
		},

		/** Use this to get the admin data object from a table row.  This will work
		* with the Simple table model.
		*/
		getTableRowDO: function (table, row) {
			return table.getTableModel().getRowData(row).adminDataObject;
		},
		...
		*/
		...
		verifyAllFields: function (theThis) {
			...
			for (var i = 0, l = theThis.fieldVerificationList.length; i < l; i++) {
				...
				var verifyResult = 0;
				if (obj.type === "string") {
					verifyResult = admin.Statics.verifyStringField(theThis, obj);
				} else if (obj.type === "int") {
					verifyResult = admin.Statics.verifyIntegerField(theThis, obj);
				} else {
				...
				if (verifyResult === -1) {
					...
					var our_tab = admin.Statics.findQXParent(field, qx.ui.tabview.Page);
		...
		verifyStringField: function (theThis, verifyObj) {
			...
			if (minLen > 0 || required) {
				if (admin.Statics.verifyTextField(field, verifyObj.label) === -1) return -1;
			}

			// Ensure that the field has a minimum length
			var val = qx.lang.String.trimRight(field.getValue());
			if (minLen > 0 && val.length < minLen) {
				admin.Statics.doAlert(verifyObj.label + " field must be at least " + verifyObj.min + " characters.",
			...
			if (maxLen > 0 && val.length > maxLen) {
				admin.Statics.doAlert(verifyObj.label + " field must not be longer than " + verifyObj.max + " characters.",
		...
		verifyIntegerField: function (theThis, verifyObj) {
			...
			var val = -1;
			if (required) {
				var val = admin.Statics.verifyInteger(field, verifyObj.label);
			}
			...
			if (verifyObj.min !== undefined &&
				...
				admin.Statics.doAlert(verifyObj.label + " field must be at least " + verifyObj.min + " in value.",
			...
			if (verifyObj.max !== undefined &&
				...
				admin.Statics.doAlert(verifyObj.label + " field must not be larger than " + verifyObj.max + ".",
		...
		verifyTextField: function (field, name) {
			if (!field.getValue() || qx.lang.String.trimRight(field.getValue()).length === 0) {
				admin.Statics.doAlert(name + " field is empty.  Please enter a value",
		...
		verifyDropDown: function (field, name) {
			if (admin.Statics.isDropDownUnSelected(field)) {
				admin.Statics.doAlert(name + " field is empty.  Please enter a value",
					name + " field is empty!", function () {
						field.focus();
					});
		...
		verifyInteger: function (field, name) {
			...
			if (admin.Statics.verifyTextField(field, name) === -1) return -1;
			...
			if (isNaN(intValue)) {
				admin.Statics.doAlert(name + " field has an invalid integer.  Please enter a valid integer.",
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						...
						admin.Statics.loadOneField(theThis, dataObject, m);
					}
				}
			}
		...
		*/
		loadOneField: function (theThis, dataObject, fieldName) {
			...
			if (formField instanceof qx.ui.form.TextField ||
				...
				admin.Statics.setDropDownByValue(formField, String(dataObject[getName]()));
		...
		pullAllFields: function (theThis, dataObject) {
			...
			for (var m in theThis) {
				if (theThis.hasOwnProperty(m)) {
					if (m.substr(m.length - 5, m.length) === "Field") {
						// This is a member that ends in "Field".  Load data for it:
						admin.Statics.pullOneField(theThis, dataObject, m);
					}
				}
			}
		...
		*/
		...
		isDevVersion: function () {
			if (admin.Singleton.getInstance().getUserProperties()["adminClientVersion"].substr(0, 4) === "314.") {
				return true;
			} else {
				return false;
			}
		...
		areWeHomeBase: function () {
			if (window.location.hostname === "admin.hericus.com") {
				return true;
			} else {
				return false;
			}
		...
		*/
		...
		decryptPwd: function (enc) {
			...
			for (var i = 0; i < enc.length; i++) {
				...
				// char not found
				if (done) {
					admin.Statics.doAlert("A password contains an invalid character. The field will be cleared in the form.  If you save the record without providing a new password, the record will be saved with the cleared password.");
					return "";
				}
			}
			return temp;
		...
		*/
		addToToolbarWithShortcut: function (toolbar, icon, tooltip, action, action_this, ownerObj, shortcut, label) {
			...
			toolbar.add(button);
			admin.Statics.setHtmlID( button, label);
		...
		isActiveEditor : function (widget) {
			var page = admin.Statics.findQXParent(widget, qx.ui.tabview.Page);
