...
/** This class defines a series of static methods that make it straightforward for
  ...
  */
qx.Class.define("admin.DataBinding", {
	...
	statics : {
		...
		bindLayout : function ( theThis, dataObject, layoutDoc) {
			...
			admin.DataBinding.bindElementAndChildren( theThis, dataObject, elem);
		...
		bindElementAndChildren : function (theThis, dataObject, elem ) {
			...
			if(elem.nodeName === "VerticalBoxLayout"){
				admin.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "HorizontalBoxLayout"){
				admin.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "VerticalSplitPane") {
				admin.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "GroupBox") {
				admin.DataBinding.bindGroupBox( theThis, dataObject, elem );
			} else if(elem.nodeName === "TreeVirtual" ) {
				admin.DataBinding.bindTreeVirtual( theThis, dataObject, elem);
			} else if(elem.nodeName === "RecursiveCheckBoxTree" ) {
				admin.DataBinding.bindRecursiveCheckBoxTree( theThis, dataObject, elem );
			} else if(elem.nodeName === "HorizontalSplitPane") {
				admin.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "TextArea") {
				admin.DataBinding.bindTextArea( theThis, dataObject, elem );
			} else if(elem.nodeName === "HtmlEmbed") {
				admin.DataBinding.bindHtmlEmbed( theThis, dataObject, elem );
			} else if(elem.nodeName === "IFrame") {
				admin.DataBinding.bindIFrame( theThis, dataObject, elem );
			} else if(elem.nodeName === "StandardTable") {
				admin.DataBinding.bindStandardTable( theThis, dataObject, elem );
			} else if(elem.nodeName === "StandardTreeVirtual") {
				admin.DataBinding.bindStandardTreeVirtual( theThis, dataObject, elem );
			} else if(elem.nodeName === "FieldArea") {
				admin.DataBinding.bindFieldArea( theThis, dataObject, elem );
			} else if(elem.nodeName === "Label") {
				admin.DataBinding.bindLabel( theThis, dataObject, elem );
			} else if(elem.nodeName === "Spacer") {
				admin.DataBinding.processChildren( theThis, dataObject, elem ); // just recurse to the children
			} else if(elem.nodeName === "List") {
				admin.DataBinding.bindList( theThis, dataObject, elem );
			} else if(elem.nodeName === "Button") {
				admin.DataBinding.bindButton( theThis, dataObject, elem );
			}
		},
		...
		bindGroupBox : function ( theThis, dataObject, elem) {
			...
			*/
			...
			admin.DataBinding.processChildren( theThis, dataObject, elem );
		},
		...
		bindLabel : function ( theThis, dataObject, elem) {
			...
			admin.DataBinding.processAttributes( theThis, elem, obj );
			return obj;
		},
		...
		bindField : function ( theThis, dataObject, elem, index ) {
			...
			var oc = admin.DataBinding.getObjectController( theThis, dataObject );
		...
		bindFieldArea : function ( theThis, dataObject, elem ) {
			...
			for(var i = 0, l = elem.childNodes.length; i < l; i ++){
				...
				if (node.nodeName === "QuadField") {
					admin.DataBinding.bindField( theThis, dataObject, node, 1);
					admin.DataBinding.bindField( theThis, dataObject, node, 2);
					admin.DataBinding.bindField( theThis, dataObject, node, 3);
					admin.DataBinding.bindField( theThis, dataObject, node, 4);
					...
					admin.Statics.addCheckboxes(theThis, parent, first,
                        ...
                        // Set read only state:
                    	admin.DataBinding.setReadOnly(theThis, node, "readOnly1", "varName1");
                		admin.DataBinding.setReadOnly(theThis, node, "readOnly2", "varName2");
		...
		processChildren : function ( theThis, dataObject, elem ){
			for(var i = 0, l = elem.childNodes.length; i < l; i++){
				var node = elem.childNodes[i];
				admin.DataBinding.bindElementAndChildren( theThis, dataObject, node );
			}
		}
	}
});
