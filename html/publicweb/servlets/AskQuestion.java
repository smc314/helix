...
{ 
    	...
    	throws ServletException, IOException 
    {
        ...
        try {
            ...
            Document doc = newDocument("TaskAddNew"); 

			Element input_root = doc.getDocumentElement();
			
			// Create the Tasks
			Element task = doc.createElement("Task");
			input_root.appendChild(task);
			task.setAttribute("id", "0");
			task.setAttribute("componentid", "25" ); // Hericus Web Site - General Web Site
			task.setAttribute("found_in_versionid", "18" ); // Hericus Web Site - 1.0
			task.setAttribute("target_versionid", "18" ); // Hericus Web Site - 1.0
			task.setAttribute("typeid", "3" ); // FAQ
			task.setAttribute("statusid", "2" ); // Review
			task.setAttribute("severityid", "3" ); // High
			task.setAttribute("owner", "9" ); // Support User
			task.setAttribute("qaowner", "9" ); // Support User
			task.setAttribute("subject", "Question from " + req.getParameter("contactemail") + " on " + new Date() );
			Element msg = doc.createElement("TaskDescription");
			task.appendChild(msg);
			setCDATASection(doc, msg, 
				"From " + req.getParameter("contactname") + " on " + new Date() + "\n" +
				"\n" +
				req.getParameter("questioncontent") );
            
			input_root.setAttribute("user", "webvisit");
			input_root.setAttribute("pass", "w3bv1s1t");
			input_root.setAttribute("dbname", "Zed");
			Document serverResp = sendToZedServer("zed.hericus.com", 443, doc, "TaskAddNew", true);
			m_out.println("<b>");
			try {
				returnHasErrors(serverResp, true);
			} catch (Exception e){
			}
			m_out.println("Your question has been forwarded to <a href=\"mailto:support@hericus.com\">Hericus Support</a> and will be responded to promptly. ");
			m_out.println("<br/>");
			m_out.println("<br/>");
			m_out.println("Thank you for contacting Hericus Software about Zed Builds And Bugs!");
			m_out.println("<br/><br/>");
	...
	/**
	 * It sets a CDATA section in the targetNode.
	 * 
	 * @param xmlDoc
	 * @param targetNode
	 * @param sBuffer
	 */
	public static void setCDATASection(Document xmlDoc, Node targetNode,
			String sBuffer)
	{
		try {
			if (targetNode == null)
			...
			Node cdataNode = targetNode.getFirstChild();
			if (cdataNode == null) {
				CDATASection cdata = xmlDoc.createCDATASection(sBuffer);
				targetNode.appendChild(cdata);
			} else if (cdataNode.getNodeType() == Node.CDATA_SECTION_NODE) {
				cdataNode.setNodeValue(sBuffer);
			}
		} catch (Exception e) {
		}
	}
